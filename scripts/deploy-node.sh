...
EXCLUDE_NON_STATE_INSTANCE_FAMILIES="${SEARCHER_EXCLUDE_NON_STATE_INSTANCE_FAMILIES:-0}"
case "$EXCLUDE_NON_STATE_INSTANCE_FAMILIES" in
  0|1) ;;
  *) say "ABORT: SEARCHER_EXCLUDE_NON_STATE_INSTANCE_FAMILIES must be 0 or 1"; exit 9 ;;
esac
STARTUP_BANNER_TIMEOUT_SECONDS="${SEARCHER_STARTUP_BANNER_TIMEOUT_SECONDS:-1200}"
...
DEFAULT_BLOCKSCAN_ROUTE_EVENTS_PATH=/var/log/mev/events/searcher-live.blockscan-routes.jsonl
ANVIL_PORT=8555
...
if [ -f "$REPO/.bribe-all-above-gas" ]; then
  abort_runtime \
    "obsolete .bribe-all-above-gas marker would donate all retained EV; remove it before deploy"
fi
AB_ROOT=/opt/MEV-ab
...
if [ -n "$PID" ] && [ "$PID" != "0" ] && [ -r "/proc/$PID/environ" ]; then
  ...
  RUNNING_BLOCKSCAN_ROUTE_EVENTS_PATH=$(tr '\0' '\n' < "/proc/$PID/environ" \
    | sed -n 's/^SEARCHER_BLOCKSCAN_ROUTE_EVENTS_PATH=//p' | tail -1)
  BLOCKSCAN_ROUTE_EVENTS_PATH=${SEARCHER_BLOCKSCAN_ROUTE_EVENTS_PATH:-${RUNNING_BLOCKSCAN_ROUTE_EVENTS_PATH:-$DEFAULT_BLOCKSCAN_ROUTE_EVENTS_PATH}}
  RUNNING_BLOCKSCAN_STATE_MULTICALL=$(process_env_value SEARCHER_BLOCKSCAN_STATE_MULTICALL "$PID")
  BLOCKSCAN_STATE_MULTICALL=${SEARCHER_BLOCKSCAN_STATE_MULTICALL:-${RUNNING_BLOCKSCAN_STATE_MULTICALL:-0}}
  case "$BLOCKSCAN_STATE_MULTICALL" in
    0|1) ;;
    *) abort_runtime "SEARCHER_BLOCKSCAN_STATE_MULTICALL must be 0 or 1" ;;
  esac
  RUNNING_PROTOCOL_TOUCH_MODE=$(process_env_value SEARCHER_BLOCKSCAN_PROTOCOL_TOUCH_MODE "$PID")
  ...
  echo "SEARCHER_BLOCKSCAN_ROUTE_EVENTS_PATH=$BLOCKSCAN_ROUTE_EVENTS_PATH" >> "$tmp"
  echo "SEARCHER_LIVE_RPC_URL=$LOCAL_RPC" >> "$tmp"
  echo "SEARCHER_LIVE_WS_URL=$LOCAL_WS" >> "$tmp"
  echo "SEARCHER_DRY_RUN=$DRY_VAL" >> "$tmp"
  # protocol-edges is marker-controlled by $REPO/.protocol-edges.
  ...
  # N-1 is a reversible degraded coarse-pricing diagnostic. Source-N code stays
  # available; removing the marker restores the normal current-N path.
  [ -f "$N_MINUS_ONE_MARKER" ] && echo "SEARCHER_BLOCKSCAN_N_MINUS_ONE_FALLBACK=1" >> "$tmp"
  ...
  echo "SEARCHER_BLOCKSCAN_STATE_MULTICALL=$BLOCKSCAN_STATE_MULTICALL" >> "$tmp"
  echo "SEARCHER_BLOCKSCAN_PROTOCOL_TOUCH_MODE=$PROTOCOL_TOUCH_MODE" >> "$tmp"
  echo "SEARCHER_EXCLUDE_NON_STATE_INSTANCE_FAMILIES=$EXCLUDE_NON_STATE_INSTANCE_FAMILIES" >> "$tmp"
  ...
  EXISTING_BLOCKSCAN_ROUTE_EVENTS_PATH=$(sed -n 's/^SEARCHER_BLOCKSCAN_ROUTE_EVENTS_PATH=//p' "$ENVF" 2>/dev/null | tail -1)
  ...
  EXISTING_BLOCKSCAN_STATE_MULTICALL=$(env_value SEARCHER_BLOCKSCAN_STATE_MULTICALL "$ENVF")
  BLOCKSCAN_STATE_MULTICALL=${SEARCHER_BLOCKSCAN_STATE_MULTICALL:-${EXISTING_BLOCKSCAN_STATE_MULTICALL:-0}}
  case "$BLOCKSCAN_STATE_MULTICALL" in
    0|1) ;;
    *) abort_runtime "SEARCHER_BLOCKSCAN_STATE_MULTICALL must be 0 or 1" ;;
  esac
  EXISTING_PROTOCOL_TOUCH_MODE=$(env_value SEARCHER_BLOCKSCAN_PROTOCOL_TOUCH_MODE "$ENVF")
  ...
  [ -f "$N_MINUS_ONE_MARKER" ] && echo "SEARCHER_BLOCKSCAN_N_MINUS_ONE_FALLBACK=1" >> "$tmp"  # reversible degraded coarse-pricing diagnostic
...
[ "$BLOCKSCAN_ROUTE_EVENTS_PATH" != "$EVENTS_PATH" ] \
  || abort_runtime "block-scan route sidecar must differ from SEARCHER_EVENTS_PATH"
...
if [ -f "$N_MINUS_ONE_MARKER" ]; then
  [ "$(process_env_count SEARCHER_BLOCKSCAN_N_MINUS_ONE_FALLBACK "$NEWPID")" = "1" ] \
    && [ "$PROCESS_N_MINUS_ONE" = "1" ] \
    || abort_runtime "N-1 marker is present but fallback is not uniquely enabled"
else
  [ "$(process_env_count SEARCHER_BLOCKSCAN_N_MINUS_ONE_FALLBACK "$NEWPID")" = "0" ] \
    || abort_runtime "N-1 fallback remained enabled without its marker"
fi
...
if [ -f "$REPO/.block-scan" ]; then
  tail -c "+$((LOG_OFFSET + 1))" "$LOGF" 2>/dev/null \
    | grep -F "[searcher/live] block-scan route events emit → $BLOCKSCAN_ROUTE_EVENTS_PATH" >/dev/null \
    || abort_runtime \
