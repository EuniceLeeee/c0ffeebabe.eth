...
function decodeIdentity(value: unknown, path: string): CurveIdentityV1 {
  ...
  const directions = Object.freeze(fieldArray(facts.verifiedDirections, (item, itemPath) => {
    const direction = exact(item, ["amountIn", "amountOut", "i", "j"], itemPath);
    return Object.freeze({ i: integer(direction.i, `${itemPath}.i`), j: integer(direction.j, `${itemPath}.j`), amountIn: decimal(direction.amountIn, `${itemPath}.amountIn`), amountOut: decimal(direction.amountOut, `${itemPath}.amountOut`) });
...
function decodeStateFact(value: unknown, path = "curve-underlying.stateFact"): CurveStateReadFactsV1 {
  ...
  const optionalKeys = ["exactAmountOut"] as const;
  ...
  return Object.freeze({ cutoff: cutoff(read.cutoff, `${path}.read.cutoff`), pool: address(read.pool, `${path}.read.pool`), variant, A: decimal(read.A, `${path}.read.A`), fee: decimal(read.fee, `${path}.read.fee`), balances: Object.freeze(fieldArray(read.balances, (item, itemPath) => decimal(item, itemPath), `${path}.read.balances`)), rates: Object.freeze(fieldArray(read.rates, (item, itemPath) => decimal(item, itemPath), `${path}.read.rates`)), ...(variant === "ng" ? { offpegFeeMultiplier: decimal(read.offpegFeeMultiplier, `${path}.read.offpegFeeMultiplier`) } : {}), ...(read.exactAmountOut === undefined ? {} : { exactAmountOut: decimal(read.exactAmountOut, `${path}.read.exactAmountOut`) }) });
}
...
const factory: FamilySearchAdapterFactoryV1 = input => {
  ...
  const readState: FamilySearchAdapterV1["readState"] = async request => {
    try {
      ...
      const exactPhysical = encodeCurveStateCall("getDyUnderlying", pool, [ctx.protocolRoute.i, ctx.protocolRoute.j, ctx.amount.amountIn]);
