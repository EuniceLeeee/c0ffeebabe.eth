...
function decodeHistory(execution: FamilySourcePlanNominationInputV1["execution"], sourceEvidence: FamilySourcePlanNominationInputV1["sourceEvidence"], rawEvidence: FamilyRawEvidenceReadPortV1): readonly HistoryChunkV1[] {
  ...
  const chunks = sourceEvidence.refs.map(evidence => {
    ...
    if (typeof range.from !== "string" || typeof range.through !== "string" || observed.familyDefinitionHash !== EIGENPIE_FAMILY_AUTHORING_HASH || observed.plan.familyDefinitionHash !== EIGENPIE_FAMILY_AUTHORING_HASH || observed.plan.ownerRef !== evidence.ownerRef || observed.plan.sourcePlanRef !== evidence.sourcePlanRef || observed.plan.completeness !== "contiguous-history" || observed.plan.historyStartBlock !== "0" || observed.requestSchemaHash !== EIGENPIE_HISTORY_SOURCE_PLAN_SCHEMA_HASH || observed.request.kind !== "family-source-plan-rpc" || observed.request.version !== 1 || observed.request.method !== "eth_getLogs" || observed.request.target !== null || observed.request.manager !== null || observed.request.topic !== EIGENPIE_ASSET_DEPOSIT_TOPIC || observed.cutoff.chainId !== execution.cutoff.chainId || BigInt(observed.cutoff.number) > BigInt(execution.cutoff.number)) throw new TypeError("eigenpie history predecessor chunk binding mismatch");
  ...
  const expected = { kind: "eigenpie-asset-deposit-contiguous-history", version: 1, topic: EIGENPIE_ASSET_DEPOSIT_TOPIC, from: "0", through: execution.through, chunkBlocks: CHUNK_BLOCKS.toString(), entries: chunks.flatMap(chunk => chunk.entries) };
...
export const EIGENPIE_HISTORY_SOURCE_PLAN_RUNTIME: FamilySourcePlanRuntimeV1 = Object.freeze({
  ...
  async execute(input: FamilySourcePlanExecutionInputV1, physical: FamilySourcePlanPhysicalPortV1, signal: AbortSignal) {
    ...
    if (input.plan.familyDefinitionHash !== EIGENPIE_FAMILY_AUTHORING_HASH || input.plan.completeness !== "contiguous-history" || input.plan.historyStartBlock !== "0") throw new TypeError("eigenpie history source plan binding mismatch");
    const predecessor = input.predecessor ?? null;
    if ((input.previousAppliedThrough === null) !== (predecessor === null)) throw new TypeError("eigenpie history durable predecessor mismatch");
    if (predecessor !== null && (predecessor.execution.through !== input.previousAppliedThrough || predecessor.execution.outcome !== "complete" || encodeCanonicalJson(predecessor.execution.plan) !== encodeCanonicalJson(input.plan))) throw new TypeError("eigenpie history predecessor lineage mismatch");
    const predecessorChunks = predecessor === null ? Object.freeze([] as HistoryChunkV1[]) : decodeHistory(predecessor.execution, predecessor.sourceEvidence, predecessor.rawEvidence);
    ...
    const opaqueResult: CanonicalJson = Object.freeze({ kind: "eigenpie-asset-deposit-contiguous-history", version: 1, topic: EIGENPIE_ASSET_DEPOSIT_TOPIC, from: "0", through: input.cutoff.number, chunkBlocks: CHUNK_BLOCKS.toString(), entries: canonicalEntries });
    const resultPartitionRoot = hashDomain("aloha/eigenpie/history-source-partition/v1", opaqueResult);
    const withoutRoot = { kind: "source-plan-execution" as const, version: 1 as const, plan: input.plan, cutoff: input.cutoff, outcome: "complete" as const, from: "0", through: input.cutoff.number, previousAppliedThrough: input.previousAppliedThrough, resultPartitionRoot, opaqueResult, sourceEvidenceRefs: refs, rawLocatorHashes, sourceEvidenceRoot: evidenceRoot };
