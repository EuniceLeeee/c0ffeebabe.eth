...
import { decodeSelfBurnNativeCandidate } from "./discovery.ts";
import { nominateSelfBurnNative } from "./nomination.ts";
import type {
...
import { SELF_BURN_NATIVE_FAMILY_ID, SELF_BURN_NATIVE_SOURCE_PLAN_ID, SELF_BURN_NATIVE_SOURCE_PLAN_SCHEMA_HASH } from "./manifest.ts";
import { SELF_BURN_NATIVE_FAMILY_AUTHORING_HASH } from "./family-definition.ts";

export const SELF_BURN_NATIVE_SOURCE_PLAN = defineFamilySourcePlan({
...
function assertNominationBinding(input: FamilySourcePlanNominationInputV1): void {
  ...
  if (
    input.execution.plan.familyDefinitionHash !== SELF_BURN_NATIVE_FAMILY_AUTHORING_HASH
  ...
  ) throw new TypeError("self-burn-native nomination binding mismatch");
}
...
export const SELF_BURN_NATIVE_SOURCE_PLAN_RUNTIME: FamilySourcePlanRuntimeV1 = Object.freeze({
  ...SELF_BURN_NATIVE_SOURCE_PLAN,
  async execute(input: FamilySourcePlanExecutionInputV1, _physical: FamilySourcePlanPhysicalPortV1, signal: AbortSignal) {
    if (signal.aborted) throw signal.reason;
    if (input.plan.familyDefinitionHash !== SELF_BURN_NATIVE_FAMILY_AUTHORING_HASH || input.plan.completeness !== "nomination-only" || input.plan.historyStartBlock !== null || input.previousAppliedThrough !== null) {
      throw new TypeError("self-burn-native source plan binding mismatch");
    }
    return sealPositiveOnlySourceExecution(input);
  },
});

export const SELF_BURN_NATIVE_SOURCE_NOMINATION_PROGRAM: FamilySourcePlanNominationProgramV1 = Object.freeze({
  ...
  async evaluate(input: FamilySourcePlanNominationInputV1, signal: AbortSignal): Promise<readonly CandidateNominationV1[]> {
    ...
    for (const evidence of input.recent.evidence) {
      ...
      const raw = decodeEvmLogObservationBytes(rawBytes, "self-burn-native.rawEvidence");
      if (
      ...
      ) throw new TypeError("self-burn-native raw evidence/recent evidence mismatch");
      ...
      const seed = decodeSelfBurnNativeCandidate(observation, "self-burn-native-call");
      if (seed === null) continue;
      const target = seed.target;
      if (seenTargets.has(target)) continue;
      seenTargets.add(target);
      const nomination = nominateSelfBurnNative({ target, evidence: observation });
      if (nomination.status !== "nominated") continue;
      nominations.push(Object.freeze({
        kind: "aloha.candidate-nomination" as const, version: "2" as const, familyId: SELF_BURN_NATIVE_FAMILY_ID, familyDefinitionHash: SELF_BURN_NATIVE_FAMILY_AUTHORING_HASH, instanceNominationKey: nomination.candidate.instanceNominationKey, evidence,
      }));
    }
