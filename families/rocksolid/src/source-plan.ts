...
import { ROCKSOLID_FAMILY_ID, ROCKSOLID_SOURCE_PLAN_ID, ROCKSOLID_SOURCE_PLAN_SCHEMA_HASH } from "./manifest.ts";
import { ROCKSOLID_FAMILY_AUTHORING_HASH } from "./family-definition.ts";

export const ROCKSOLID_SOURCE_PLAN = defineFamilySourcePlan({
  sourcePlanId: ROCKSOLID_SOURCE_PLAN_ID,
  completeness: "nomination-only",
  historyStartBlock: null,
  schemaHash: ROCKSOLID_SOURCE_PLAN_SCHEMA_HASH,
});
...
function assertNominationBinding(input: FamilySourcePlanNominationInputV1): void {
  if (
    input.execution.plan.familyDefinitionHash !== ROCKSOLID_FAMILY_AUTHORING_HASH
  ...
  ) throw new TypeError("rocksolid nomination binding mismatch");
}
...
export const ROCKSOLID_SOURCE_PLAN_RUNTIME: FamilySourcePlanRuntimeV1 = Object.freeze({
  ...ROCKSOLID_SOURCE_PLAN,
  async execute(input: FamilySourcePlanExecutionInputV1, _physical: FamilySourcePlanPhysicalPortV1, signal: AbortSignal) {
    if (signal.aborted) throw signal.reason;
    if (input.plan.familyDefinitionHash !== ROCKSOLID_FAMILY_AUTHORING_HASH || input.plan.completeness !== "nomination-only" || input.plan.historyStartBlock !== null || input.previousAppliedThrough !== null) {
      throw new TypeError("rocksolid source plan binding mismatch");
    }
    return sealPositiveOnlySourceExecution(input);
  },
});

export const ROCKSOLID_SOURCE_NOMINATION_PROGRAM: FamilySourcePlanNominationProgramV1 = Object.freeze({
