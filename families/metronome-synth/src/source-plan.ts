...
import { METRONOME_SYNTH_FAMILY_ID, METRONOME_SYNTH_SOURCE_PLAN_ID, METRONOME_SYNTH_SOURCE_PLAN_SCHEMA_HASH } from "./manifest.ts";
import { METRONOME_SYNTH_FAMILY_AUTHORING_HASH } from "./family-definition.ts";

export const METRONOME_SYNTH_SOURCE_PLAN = defineFamilySourcePlan({
  sourcePlanId: METRONOME_SYNTH_SOURCE_PLAN_ID,
  completeness: "nomination-only",
  historyStartBlock: null,
  schemaHash: METRONOME_SYNTH_SOURCE_PLAN_SCHEMA_HASH,
});
...
function assertNominationBinding(input: FamilySourcePlanNominationInputV1): void {
  if (
    input.execution.plan.familyDefinitionHash !== METRONOME_SYNTH_FAMILY_AUTHORING_HASH
  ...
  ) throw new TypeError("metronome-synth nomination binding mismatch");
}
...
export const METRONOME_SYNTH_SOURCE_PLAN_RUNTIME: FamilySourcePlanRuntimeV1 = Object.freeze({
  ...METRONOME_SYNTH_SOURCE_PLAN,
  async execute(input: FamilySourcePlanExecutionInputV1, _physical: FamilySourcePlanPhysicalPortV1, signal: AbortSignal) {
    if (signal.aborted) throw signal.reason;
    if (input.plan.familyDefinitionHash !== METRONOME_SYNTH_FAMILY_AUTHORING_HASH || input.plan.completeness !== "nomination-only" || input.plan.historyStartBlock !== null || input.previousAppliedThrough !== null) {
      throw new TypeError("metronome-synth source plan binding mismatch");
    }
    return sealPositiveOnlySourceExecution(input);
  },
});

export const METRONOME_SYNTH_SOURCE_NOMINATION_PROGRAM: FamilySourcePlanNominationProgramV1 = Object.freeze({
