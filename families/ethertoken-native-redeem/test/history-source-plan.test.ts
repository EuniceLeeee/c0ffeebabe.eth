...
import { ETHERTOKEN_NATIVE_REDEEM_DESTRUCTION_TOPIC } from "../src/manifest.ts";
import { ETHERTOKEN_NATIVE_REDEEM_HISTORY_NOMINATION_PROGRAM, ETHERTOKEN_NATIVE_REDEEM_HISTORY_SOURCE_PLAN_RUNTIME } from "../src/history-source-plan.ts";

const h = (value: string): Hash => hashDomain("test/ethertoken-native-redeem-history/v1", value);
...
test("EtherToken history preserves genesis-through-cutoff bytes across a durable predecessor", async () => {
  const target = address("a");
  const first = await ETHERTOKEN_NATIVE_REDEEM_HISTORY_SOURCE_PLAN_RUNTIME.execute({ plan, cutoff: cutoff("50"), previousAppliedThrough: null }, physical(() => [destruction(target, 2n)]), new AbortController().signal);
  const second = await ETHERTOKEN_NATIVE_REDEEM_HISTORY_SOURCE_PLAN_RUNTIME.execute({ plan, cutoff: cutoff("52"), previousAppliedThrough: "50", predecessor: { persistedExecutionRoot: h("persisted"), execution: first.execution, sourceEvidence: first.sourceEvidence, rawEvidence: rawPort(first.rawEvidenceLocators) } }, physical(() => []), new AbortController().signal);
  ...
  const nominations = await ETHERTOKEN_NATIVE_REDEEM_HISTORY_NOMINATION_PROGRAM.evaluate({ execution: second.execution, sourceEvidence: second.sourceEvidence, recent: recent("52"), rawEvidence: rawPort(second.rawEvidenceLocators) }, new AbortController().signal);
  assert.deepEqual(nominations.map(value => value.instanceNominationKey), [target]);
});

test("EtherToken history rejects a forged source-plan physical observation", async () => {
  await assert.rejects(() => ETHERTOKEN_NATIVE_REDEEM_HISTORY_SOURCE_PLAN_RUNTIME.execute({ plan, cutoff: cutoff("5"), previousAppliedThrough: null }, physical(() => [], observation => { observation.familyDefinitionHash = h("forged"); }), new AbortController().signal), /binding mismatch/);
});
