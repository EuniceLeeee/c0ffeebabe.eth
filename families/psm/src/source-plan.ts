...
import { PSM_FAMILY_ID, PSM_SOURCE_PLAN_ID, PSM_SOURCE_PLAN_SCHEMA_HASH } from "./manifest.ts";
import { PSM_FAMILY_AUTHORING_HASH } from "./family-definition.ts";

export const PSM_SOURCE_PLAN = defineFamilySourcePlan({
...
function assertNominationBinding(input: FamilySourcePlanNominationInputV1): void {
  if (
    input.execution.plan.familyDefinitionHash !== PSM_FAMILY_AUTHORING_HASH
  ...
  ) throw new TypeError("psm nomination binding mismatch");
}
...
export const PSM_SOURCE_PLAN_RUNTIME: FamilySourcePlanRuntimeV1 = Object.freeze({
  ...PSM_SOURCE_PLAN,
  async execute(input: FamilySourcePlanExecutionInputV1, _physical: FamilySourcePlanPhysicalPortV1, signal: AbortSignal) {
    if (signal.aborted) throw signal.reason;
    if (input.plan.familyDefinitionHash !== PSM_FAMILY_AUTHORING_HASH || input.plan.completeness !== "nomination-only" || input.plan.historyStartBlock !== null || input.previousAppliedThrough !== null) {
      throw new TypeError("psm source plan binding mismatch");
    }
    return sealPositiveOnlySourceExecution(input);
  },
});

export const PSM_SOURCE_NOMINATION_PROGRAM: FamilySourcePlanNominationProgramV1 = Object.freeze({
