...
import { METRONOME_HGUSDC_FAMILY_ID, METRONOME_HGUSDC_SOURCE_PLAN_ID, METRONOME_HGUSDC_SOURCE_PLAN_SCHEMA_HASH } from "./manifest.ts";
import { METRONOME_HGUSDC_FAMILY_AUTHORING_HASH } from "./family-definition.ts";

export const METRONOME_HGUSDC_SOURCE_PLAN = defineFamilySourcePlan({
  sourcePlanId: METRONOME_HGUSDC_SOURCE_PLAN_ID,
  completeness: "nomination-only",
  historyStartBlock: null,
  schemaHash: METRONOME_HGUSDC_SOURCE_PLAN_SCHEMA_HASH,
});
...
function assertNominationBinding(input: FamilySourcePlanNominationInputV1): void {
  if (
    input.execution.plan.familyDefinitionHash !== METRONOME_HGUSDC_FAMILY_AUTHORING_HASH
  ...
  ) throw new TypeError("metronome-hgusdc nomination binding mismatch");
}
...
export const METRONOME_HGUSDC_SOURCE_PLAN_RUNTIME: FamilySourcePlanRuntimeV1 = Object.freeze({
  ...METRONOME_HGUSDC_SOURCE_PLAN,
  async execute(input: FamilySourcePlanExecutionInputV1, _physical: FamilySourcePlanPhysicalPortV1, signal: AbortSignal) {
    if (signal.aborted) throw signal.reason;
    if (input.plan.familyDefinitionHash !== METRONOME_HGUSDC_FAMILY_AUTHORING_HASH || input.plan.completeness !== "nomination-only" || input.plan.historyStartBlock !== null || input.previousAppliedThrough !== null) {
      throw new TypeError("metronome-hgusdc source plan binding mismatch");
    }
    return sealPositiveOnlySourceExecution(input);
  },
});

export const METRONOME_HGUSDC_SOURCE_NOMINATION_PROGRAM: FamilySourcePlanNominationProgramV1 = Object.freeze({
