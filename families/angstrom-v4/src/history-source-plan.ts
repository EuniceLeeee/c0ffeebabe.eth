...
export const ANGSTROM_V4_HISTORY_SOURCE_PLAN_RUNTIME: FamilySourcePlanRuntimeV1 = Object.freeze({
  ...
  async execute(input: FamilySourcePlanExecutionInputV1, physical: FamilySourcePlanPhysicalPortV1, signal: AbortSignal) {
    ...
    if (input.plan.familyDefinitionHash !== ANGSTROM_V4_FAMILY_DEFINITION_HASH || input.plan.completeness !== "contiguous-history" || input.plan.historyStartBlock !== "0") throw new TypeError("angstrom-v4 history source plan binding mismatch");
    const from = input.previousAppliedThrough === null ? "0" : decimal(BigInt(input.previousAppliedThrough) + 1n); if (BigInt(from) > BigInt(input.cutoff.number)) throw new TypeError("angstrom-v4 history cursor beyond cutoff");
    ...
    const opaqueResult: CanonicalJson = Object.freeze({ kind: "angstrom-v4-initialize-contiguous-history", version: 1, manager: MANAGER, topic: ANGSTROM_V4_CONTRACT_EVIDENCE_TOPIC, from, through: input.cutoff.number, chunkBlocks: CHUNK_BLOCKS.toString(), entries: Object.freeze(entries) }); const resultPartitionRoot = hashDomain("aloha/angstrom-v4/history-source-partition/v1", opaqueResult); const withoutRoot = { kind: "source-plan-execution" as const, version: 1 as const, plan: input.plan, cutoff: input.cutoff, outcome: "complete" as const, from, through: input.cutoff.number, previousAppliedThrough: input.previousAppliedThrough, resultPartitionRoot, opaqueResult, sourceEvidenceRefs: refs, rawLocatorHashes, sourceEvidenceRoot: evidenceRoot };
...
export const ANGSTROM_V4_HISTORY_NOMINATION_PROGRAM: FamilySourcePlanNominationProgramV1 = Object.freeze({
  ...
  async evaluate(input: FamilySourcePlanNominationInputV1, signal: AbortSignal): Promise<readonly CandidateNominationV1[]> {
    ...
    const entries = chunks.flatMap(chunk => chunk.entries); const expected = { kind: "angstrom-v4-initialize-contiguous-history", version: 1, manager: MANAGER, topic: ANGSTROM_V4_CONTRACT_EVIDENCE_TOPIC, from: input.execution.from, through: input.execution.through, chunkBlocks: CHUNK_BLOCKS.toString(), entries }; if (encodeCanonicalJson(expected) !== encodeCanonicalJson(input.execution.opaqueResult)) throw new TypeError("angstrom-v4 history result/raw mismatch");
