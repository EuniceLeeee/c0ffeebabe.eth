...
import { ANGSTROM_V4_ACTION_PORT, buildAngstromV4Action, buildAngstromV4SearchAction } from "./action.ts";
import { coarseAngstromV4, deriveAngstromV4Routes, assertAngstromV4Route } from "./stages.ts";
import { ANGSTROM_V4_ACTION_OWNER_ID } from "./manifest.ts";
import { ANGSTROM_MAINNET_HOOK, ANGSTROM_V4_GET_LIQUIDITY_SELECTOR, ANGSTROM_V4_GET_SLOT0_SELECTOR, ANGSTROM_V4_POOL_MANAGER, ANGSTROM_V4_QUOTER, ANGSTROM_V4_STATE_VIEW, assertPoolKey, decodeWords, encodePoolIdCall, encodeQuoteCall, encodeSwapCall, poolIdForKey } from "./abi.ts";
import type { AngstromV4IdentityV1, AngstromV4PoolKey, AngstromV4QuoteV1, AngstromV4RouteV1 } from "./types.ts";
...
type Context = {
  ...
  readonly protocolRoute: AngstromV4RouteV1;
};

type StatePayload = {
  readonly kind: "angstrom-v4-state-read";
  readonly version: 1;
...
function decodeIdentity(value: unknown, path = "angstrom-v4.identityMemo.identity"): AngstromV4IdentityV1 {
  ...
  if (
    ...
    || poolKey.hooks !== ANGSTROM_MAINNET_HOOK.toLowerCase()
    || target !== ANGSTROM_V4_POOL_MANAGER.toLowerCase()
    || managerBinding.manager !== target
    || managerBinding.stateView !== ANGSTROM_V4_STATE_VIEW.toLowerCase()
    || managerBinding.quoter !== ANGSTROM_V4_QUOTER.toLowerCase()
    || (inputAsset !== poolKey.currency0 && inputAsset !== poolKey.currency1)
    || (outputAsset !== poolKey.currency0 && outputAsset !== poolKey.currency1)
    || inputAsset === outputAsset
  ) throw new TypeError("angstrom-v4 strict PoolKey identity binding mismatch");
  const facts = Object.freeze({ target, inputAsset, outputAsset, poolId, poolKey, managerBinding });
  const identity = Object.freeze({ cutoff: familySearchSource(source.cutoff, `${path}.cutoff`), candidateSnapshotHash: hash(source.candidateSnapshotHash, `${path}.candidateSnapshotHash`), instanceKey: text(source.instanceKey, `${path}.instanceKey`), factsHash: hash(source.factsHash, `${path}.factsHash`), facts }) as AngstromV4IdentityV1;
  if (identity.instanceKey !== poolId) throw new TypeError("angstrom-v4 identity facts invalid");
  if (identity.factsHash !== hashDomain("aloha/angstrom-v4/identity-facts/v1", identity.facts)) throw new TypeError("angstrom-v4 identity facts hash mismatch");
  return identity;
}
...
function decodeMemo(value: unknown, path = "route.identityMemo") {
  ...
  if (source.kind !== "angstrom-v4-identity-memo" || source.familyId !== "angstrom-v4") throw new TypeError("angstrom-v4 identity memo discriminator mismatch");
  const familyDefinitionHash = hash(source.familyDefinitionHash, `${path}.familyDefinitionHash`);
  if (familyDefinitionHash !== ANGSTROM_V4_FAMILY_DEFINITION_HASH) throw new TypeError("angstrom-v4 identity memo definition mismatch");
  ...
  if (instanceNominationKey !== identity.instanceKey || candidateSnapshotHash !== identity.candidateSnapshotHash || familyCandidateKey !== discoveryFamilyCandidateKey(familyDefinitionHash, instanceNominationKey)) throw new TypeError("angstrom-v4 identity memo lineage mismatch");
...
function context(input: FamilySearchLegRequestV1): Context {
  ...
  if (route.familyId !== "angstrom-v4" || route.familyDefinitionHash !== ANGSTROM_V4_FAMILY_DEFINITION_HASH) throw new TypeError("angstrom-v4 search route family mismatch");
  const memo = decodeMemo(route.identityMemo);
  if (hashDomain("aloha/identity-memo/v1", route.identityMemo) !== route.identityMemoHash || route.instanceKey !== memo.identity.instanceKey) throw new TypeError("angstrom-v4 identity memo binding mismatch");
  ...
  const protocolRoute = deriveAngstromV4Routes(memo.identity).find(item => assetRef(source.chainId, item.inputAsset) === amount.inputAssetRef && assetRef(source.chainId, item.outputAsset) === amount.outputAssetRef);
  if (protocolRoute === undefined) throw new TypeError("angstrom-v4 search amount assets do not match identity");
  assertAngstromV4Route(protocolRoute, memo.identity);
...
function unavailable(stage: "state" | "coarse" | "exact" | "action", reasonCode: string, evidence: unknown) { return unavailableFamilySearchStage(stage, `angstrom-v4-${reasonCode}`, canonical(evidence)); }
...
function stateRequest(ctx: Context, kind: "slot0" | "liquidity" | "quote") {
  ...
  const root = hashDomain("aloha/angstrom-v4/search-state-request/v2", { routeBindingHash: ctx.routeBindingHash, amountHash: ctx.amountHash, source: ctx.source });
  const requestId = hashDomain("aloha/angstrom-v4/search-state-request-part/v1", { root, kind });
  const target = kind === "slot0" || kind === "liquidity" ? binding.stateView : binding.quoter;
  const data = kind === "slot0" ? encodePoolIdCall(ANGSTROM_V4_GET_SLOT0_SELECTOR, poolId) : kind === "liquidity" ? encodePoolIdCall(ANGSTROM_V4_GET_LIQUIDITY_SELECTOR, poolId) : encodeQuoteCall(poolKey, ctx.protocolRoute.inputAsset === poolKey.currency0, ctx.amount.amountIn);
  return Object.freeze({ request: Object.freeze({ kind: "family-search.current-source-read" as const, requestId, source: ctx.source, target, data, responseEncoding: `abi-angstrom-v4-${kind}` as `abi-${string}` }), requestId, kind });
}
...
function readResult(result: FamilySearchSourceReadResultV1, expected: ReturnType<typeof stateRequest>, source: Context["source"]): string {
  ...
  if (result.requestId !== expected.requestId || !sameFamilySearchSource(result.source, source)) throw new TypeError("angstrom-v4 source response binding mismatch");
  return bytes(result.dataHex, "angstrom-v4 source return data");
}

function decodeStatePayload(value: unknown, path = "angstrom-v4.state.payload"): StatePayload {
  ...
  if (source.kind !== "angstrom-v4-state-read" || source.version !== 1) throw new TypeError("angstrom-v4 state payload discriminator mismatch");
  ...
  return Object.freeze({ kind: "angstrom-v4-state-read", version: 1, cutoff: familySearchSource(source.cutoff, `${path}.cutoff`), routeBindingHash: hash(source.routeBindingHash, `${path}.routeBindingHash`), amountHash: hash(source.amountHash, `${path}.amountHash`), instanceKey: text(source.instanceKey, `${path}.instanceKey`), poolId: hash(source.poolId, `${path}.poolId`), poolKey: assertPoolKey(source.poolKey, `${path}.poolKey`), managerBinding, slot0ReturnDataHex: bytes(source.slot0ReturnDataHex, `${path}.slot0ReturnDataHex`), liquidityReturnDataHex: bytes(source.liquidityReturnDataHex, `${path}.liquidityReturnDataHex`), quoteReturnDataHex: bytes(source.quoteReturnDataHex, `${path}.quoteReturnDataHex`), requestIds });
}

function stateFromArtifact(ctx: Context, artifact: FamilySearchStateArtifactV1): StatePayload {
  if (artifact.kind !== "state" || artifact.status !== "verified" || !sameFamilySearchSource(artifact.source, ctx.source) || artifact.routeBindingHash !== ctx.routeBindingHash) throw new TypeError("angstrom-v4 state artifact binding mismatch");
...
function stateArtifact(ctx: Context, payload: StatePayload): FamilySearchStateArtifactV1 {
  ...
  return Object.freeze({ kind: "state", status: "verified", source: ctx.source, routeBindingHash: ctx.routeBindingHash, payload: canonicalPayload, payloadHash, artifactHash: familySearchArtifactHash({ kind: "state", source: ctx.source, routeBindingHash: ctx.routeBindingHash, objectiveRef: null, amountHash: null, payloadHash }), factsRoot: hashDomain("aloha/angstrom-v4/state-facts/v2", payload), sourceRequestId: payload.requestIds[0]! });
}

function quoteFromState(ctx: Context, state: StatePayload): AngstromV4QuoteV1 {
  const slot0 = decodeWords(state.slot0ReturnDataHex, 4, "angstrom-v4.slot0");
  const liquidity = decodeWords(state.liquidityReturnDataHex, 1, "angstrom-v4.liquidity");
  const quote = decodeWords(state.quoteReturnDataHex, 2, "angstrom-v4.quote");
  if (slot0[0] === 0n || liquidity[0] === 0n || quote[0] === 0n) throw new Error("pool-inactive-at-current-source");
  const result = coarseAngstromV4({ identity: ctx.identity, route: ctx.protocolRoute, amountIn: ctx.amount.amountIn, observedAmountOut: quote[0]!.toString(10) });
  if (result.status !== "rankable") throw new Error(result.reasonCode);
  return result.quote;
}

function coarseArtifact(ctx: Context, stateArtifactValue: FamilySearchStateArtifactV1, quote: AngstromV4QuoteV1): FamilySearchCoarseArtifactV1 {
  const payload = canonical(quote);
  const payloadHash = familySearchPayloadHash("coarse", payload);
  return Object.freeze({ kind: "coarse", status: "rankable", source: ctx.source, routeBindingHash: ctx.routeBindingHash, objectiveRef: ctx.objective.objectiveRef, amountHash: ctx.amountHash, payload, payloadHash, artifactHash: familySearchArtifactHash({ kind: "coarse", source: ctx.source, routeBindingHash: ctx.routeBindingHash, objectiveRef: ctx.objective.objectiveRef, amountHash: ctx.amountHash, payloadHash }), projectionHash: hashDomain("aloha/angstrom-v4/search-coarse-projection/v1", { quoteHash: quote.quoteHash, stateFactsRoot: stateArtifactValue.factsRoot }), stateFactsRoot: stateArtifactValue.factsRoot, input: { assetRef: ctx.amount.inputAssetRef, amount: quote.amountIn }, output: { assetRef: ctx.amount.outputAssetRef, amount: quote.observedAmountOut }, conservativeOutputUpperBound: quote.observedAmountOut, inputCapacityUpperBound: null, rankKey: hashDomain("aloha/angstrom-v4/search-coarse-rank/v1", { objectiveRef: ctx.objective.objectiveRef, routeBindingHash: ctx.routeBindingHash, quoteHash: quote.quoteHash }), reasonCode: null });
}

function decodeQuote(value: unknown, path: string): AngstromV4QuoteV1 {
  const source = exact(value, ["amountIn", "cutoff", "observedAmountOut", "quoteHash", "routeBindingHash"], path);
  const quote = Object.freeze({ cutoff: familySearchSource(source.cutoff, `${path}.cutoff`) as AngstromV4QuoteV1["cutoff"], routeBindingHash: hash(source.routeBindingHash, `${path}.routeBindingHash`), amountIn: decimal(source.amountIn, `${path}.amountIn`), observedAmountOut: decimal(source.observedAmountOut, `${path}.observedAmountOut`), quoteHash: hash(source.quoteHash, `${path}.quoteHash`) });
  if (quote.quoteHash !== hashDomain("aloha/angstrom-v4/quote/v1", { cutoff: quote.cutoff, routeBindingHash: quote.routeBindingHash, amountIn: quote.amountIn, observedAmountOut: quote.observedAmountOut })) throw new TypeError("angstrom-v4 quote hash mismatch");
  return quote;
}
function validateCoarse(ctx: Context, artifact: FamilySearchCoarseArtifactV1, stateArtifactValue: FamilySearchStateArtifactV1): AngstromV4QuoteV1 {
  if (artifact.kind !== "coarse" || artifact.status !== "rankable" || !sameFamilySearchSource(artifact.source, ctx.source) || artifact.routeBindingHash !== ctx.routeBindingHash || artifact.objectiveRef !== ctx.objective.objectiveRef || artifact.amountHash !== ctx.amountHash) throw new TypeError("angstrom-v4 coarse artifact binding mismatch");
  const payload = canonical(artifact.payload);
  const payloadHash = familySearchPayloadHash("coarse", payload);
  if (artifact.payloadHash !== payloadHash || artifact.artifactHash !== familySearchArtifactHash({ kind: "coarse", source: ctx.source, routeBindingHash: ctx.routeBindingHash, objectiveRef: ctx.objective.objectiveRef, amountHash: ctx.amountHash, payloadHash }) || artifact.stateFactsRoot !== stateArtifactValue.factsRoot) throw new TypeError("angstrom-v4 coarse artifact hash mismatch");
  const quote = decodeQuote(payload, "angstrom-v4.coarse.payload");
  if (quote.routeBindingHash !== ctx.protocolRoute.routeBindingHash || quote.amountIn !== ctx.amount.amountIn || artifact.input.assetRef !== ctx.amount.inputAssetRef || artifact.output?.assetRef !== ctx.amount.outputAssetRef || artifact.output.amount !== quote.observedAmountOut || artifact.projectionHash !== hashDomain("aloha/angstrom-v4/search-coarse-projection/v1", { quoteHash: quote.quoteHash, stateFactsRoot: stateArtifactValue.factsRoot })) throw new TypeError("angstrom-v4 coarse artifact lineage mismatch");
  return quote;
}
function exactArtifact(ctx: Context, stateArtifactValue: FamilySearchStateArtifactV1, quote: AngstromV4QuoteV1): FamilySearchExactArtifactV1 {
  const payload = canonical({ quote });
  const payloadHash = familySearchPayloadHash("exact", payload);
  const evaluationHash = hashDomain("aloha/angstrom-v4/search-exact-evaluation/v1", { quoteHash: quote.quoteHash, stateFactsRoot: stateArtifactValue.factsRoot });
  const obligationRoot = hashDomain("aloha/angstrom-v4/search-obligation/v1", { evaluationHash, quoteHash: quote.quoteHash });
...
function validateExact(ctx: Context, artifact: FamilySearchExactArtifactV1): AngstromV4QuoteV1 {
  if (artifact.kind !== "exact" || artifact.status !== "verified" || !sameFamilySearchSource(artifact.source, ctx.source) || artifact.routeBindingHash !== ctx.routeBindingHash || artifact.objectiveRef !== ctx.objective.objectiveRef || artifact.amountHash !== ctx.amountHash) throw new TypeError("angstrom-v4 exact artifact binding mismatch");
  const payload = canonical(artifact.payload);
  const payloadHash = familySearchPayloadHash("exact", payload);
  if (artifact.payloadHash !== payloadHash || artifact.artifactHash !== familySearchArtifactHash({ kind: "exact", source: ctx.source, routeBindingHash: ctx.routeBindingHash, objectiveRef: ctx.objective.objectiveRef, amountHash: ctx.amountHash, payloadHash })) throw new TypeError("angstrom-v4 exact artifact hash mismatch");
  const source = exact(payload, ["quote"], "angstrom-v4.exact.payload");
  const quote = decodeQuote(source.quote, "angstrom-v4.exact.quote");
  const evaluationHash = hashDomain("aloha/angstrom-v4/search-exact-evaluation/v1", { quoteHash: quote.quoteHash, stateFactsRoot: artifact.stateFactsRoot });
  if (quote.routeBindingHash !== ctx.protocolRoute.routeBindingHash || quote.amountIn !== ctx.amount.amountIn || artifact.evaluationHash !== evaluationHash || artifact.inputs[0]?.assetRef !== ctx.amount.inputAssetRef || artifact.outputs[0]?.assetRef !== ctx.amount.outputAssetRef || artifact.obligationRoot !== hashDomain("aloha/angstrom-v4/search-obligation/v1", { evaluationHash, quoteHash: quote.quoteHash })) throw new TypeError("angstrom-v4 exact artifact lineage mismatch");
  return quote;
}
function actionArtifact(ctx: Context, exactArtifactValue: FamilySearchExactArtifactV1, quote: AngstromV4QuoteV1, ownerRef: Hash | null): FamilySearchActionArtifactV1 {
  const poolKey = ctx.identity.facts.poolKey;
  const rawAction = buildAngstromV4Action({ identity: ctx.identity, quote, calldata: encodeSwapCall(poolKey, ctx.protocolRoute.inputAsset === poolKey.currency0, quote.amountIn) });
  const action = buildAngstromV4SearchAction({ rawAction, quote, route: ctx.protocolRoute, poolKey, zeroForOne: ctx.protocolRoute.inputAsset === poolKey.currency0, stateFactsRoot: exactArtifactValue.stateFactsRoot, inputs: exactArtifactValue.inputs, outputs: exactArtifactValue.outputs, exactEvaluationHash: exactArtifactValue.evaluationHash, obligationRoot: exactArtifactValue.obligationRoot });
  const payload = canonical(action);
  const payloadHash = familySearchPayloadHash("action", payload);
  return Object.freeze({ kind: "action", status: "ready", source: ctx.source, routeBindingHash: ctx.routeBindingHash, objectiveRef: ctx.objective.objectiveRef, amountHash: ctx.amountHash, payload, payloadHash, artifactHash: familySearchArtifactHash({ kind: "action", source: ctx.source, routeBindingHash: ctx.routeBindingHash, objectiveRef: ctx.objective.objectiveRef, amountHash: ctx.amountHash, payloadHash }), actionHash: action.actionHash, exactEvaluationHash: exactArtifactValue.evaluationHash, actionOwnerId: ANGSTROM_V4_ACTION_PORT.actionOwnerId, actionOwnerRef: ownerRef, opaqueBytes: encodePackedCallProgram([{ target: rawAction.target as `0x${string}`, value: "0", calldata: rawAction.calldata as `0x${string}` }]), inputs: exactArtifactValue.inputs, outputs: exactArtifactValue.outputs, obligationRoot: exactArtifactValue.obligationRoot });
}

const factory: FamilySearchAdapterFactoryV1 = input => {
  if (input.familyDefinitionHash !== ANGSTROM_V4_FAMILY_DEFINITION_HASH) throw new TypeError("angstrom-v4 search factory definition mismatch");
  ...
  const ownerRef = input.actionOwnerRefs.action === undefined ? null : hash(input.actionOwnerRefs.action, "angstrom-v4 action owner ref");
  const readState: FamilySearchAdapterV1["readState"] = async request => {
    try {
      ...
      decodeWords(data[0]!, 4, "angstrom-v4.slot0"); decodeWords(data[1]!, 1, "angstrom-v4.liquidity"); decodeWords(data[2]!, 2, "angstrom-v4.quote");
      const payload = Object.freeze({ kind: "angstrom-v4-state-read" as const, version: 1 as const, cutoff: ctx.source, routeBindingHash: ctx.routeBindingHash, amountHash: ctx.amountHash, instanceKey: ctx.identity.instanceKey, poolId: ctx.identity.facts.poolId, poolKey: ctx.identity.facts.poolKey, managerBinding: ctx.identity.facts.managerBinding, slot0ReturnDataHex: data[0]!, liquidityReturnDataHex: data[1]!, quoteReturnDataHex: data[2]!, requestIds: Object.freeze(requests.map(item => item.requestId)) });
      const state = stateArtifact(ctx, payload);
      const slot0 = decodeWords(data[0]!, 4, "angstrom-v4.slot0"); const liquidity = decodeWords(data[1]!, 1, "angstrom-v4.liquidity"); const quote = decodeWords(data[2]!, 2, "angstrom-v4.quote");
...
export const ANGSTROM_V4_SEARCH_RUNTIME_ADAPTER_FACTORY = factory;
