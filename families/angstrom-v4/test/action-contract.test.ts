...
import { ANGSTROM_V4_POOL_MANAGER, encodeSwapCall, poolIdForKey } from "../src/abi.ts";
import { ANGSTROM_V4_ACTION_MUTATION_CORPUS_ROOT, ANGSTROM_V4_ACTION_MUTATION_EXECUTION_IDS, ANGSTROM_V4_ACTION_PORT, buildAngstromV4SearchAction } from "../src/action.ts";

const h = (value: string): Hash => hashDomain("aloha/angstrom-v4/action-contract-test/v1", value);
...
const route = Object.freeze({ ...routeBody, routeBindingHash: hashDomain("aloha/angstrom-v4/route-binding/v1", routeBody) });
const quoteBody = { cutoff, routeBindingHash: route.routeBindingHash, amountIn: "100", observedAmountOut: "97" };
const quote = Object.freeze({ ...quoteBody, quoteHash: hashDomain("aloha/angstrom-v4/quote/v1", quoteBody) });
const rawBody = { cutoff, target: ANGSTROM_V4_POOL_MANAGER.toLowerCase(), calldata: encodeSwapCall(poolKey, true, quote.amountIn), exactQuoteHash: quote.quoteHash };
const rawAction = Object.freeze({ ...rawBody, actionHash: hashDomain("aloha/angstrom-v4/action/v1", rawBody) });
const stateFactsRoot = h("facts");
const exactEvaluationHash = hashDomain("aloha/angstrom-v4/search-exact-evaluation/v1", { quoteHash: quote.quoteHash, stateFactsRoot });
const obligationRoot = hashDomain("aloha/angstrom-v4/search-obligation/v1", { evaluationHash: exactEvaluationHash, quoteHash: quote.quoteHash });
const action = buildAngstromV4SearchAction({ rawAction, quote, route, poolKey, zeroForOne: true, stateFactsRoot, inputs: [{ assetRef: erc20AssetRefV1("1", poolKey.currency0), amount: "100" }], outputs: [{ assetRef: erc20AssetRefV1("1", poolKey.currency1), amount: "97" }], exactEvaluationHash, obligationRoot });

test("Angstrom action owner exactly verifies its semantic payload and obligation proof", () => {
  assert.equal(ANGSTROM_V4_ACTION_PORT.decode(action).actionHash, action.actionHash);
  assert.equal(ANGSTROM_V4_ACTION_PORT.verifyObligations(action).subjectRoot, obligationRoot);
  const reject = (value: unknown) => assert.throws(() => ANGSTROM_V4_ACTION_PORT.decode(value));
  ...
  assert.deepEqual(mutationCorpus.map(mutation => mutation.id), ANGSTROM_V4_ACTION_MUTATION_EXECUTION_IDS);
  assert.equal(ANGSTROM_V4_ACTION_MUTATION_CORPUS_ROOT, hashDomain("aloha/angstrom-v4/action-verifier-mutations/v1", { executionIds: ANGSTROM_V4_ACTION_MUTATION_EXECUTION_IDS }));
  ...
  const foreignRoute = { ...foreignRouteBody, routeBindingHash: hashDomain("aloha/angstrom-v4/route-binding/v1", foreignRouteBody) };
  const foreignQuoteBody = { ...quoteBody, routeBindingHash: foreignRoute.routeBindingHash };
  const foreignQuote = { ...foreignQuoteBody, quoteHash: hashDomain("aloha/angstrom-v4/quote/v1", foreignQuoteBody) };
  const foreignRawBody = { ...rawBody, exactQuoteHash: foreignQuote.quoteHash };
  const foreignRaw = { ...foreignRawBody, actionHash: hashDomain("aloha/angstrom-v4/action/v1", foreignRawBody) };
  const foreignEvaluation = hashDomain("aloha/angstrom-v4/search-exact-evaluation/v1", { quoteHash: foreignQuote.quoteHash, stateFactsRoot });
  const foreignObligation = hashDomain("aloha/angstrom-v4/search-obligation/v1", { evaluationHash: foreignEvaluation, quoteHash: foreignQuote.quoteHash });
  const foreignProof = hashDomain("aloha/angstrom-v4/action-obligation-postcondition/v1", { rawActionHash: foreignRaw.actionHash, routeBindingHash: foreignRoute.routeBindingHash, exactEvaluationHash: foreignEvaluation, obligationRoot: foreignObligation, inputs: action.inputs, outputs: action.outputs, stateFactsRoot, poolKey, zeroForOne: true });
  ...
  void ignoredForeign;
  reject({ ...foreignBody, actionHash: hashDomain("aloha/angstrom-v4/search-action/v1", foreignBody) });
  const cutoffRawBody = { ...rawBody, cutoff: { ...cutoff, number: "101" } };
  const cutoffRaw = { ...cutoffRawBody, actionHash: hashDomain("aloha/angstrom-v4/action/v1", cutoffRawBody) };
  const cutoffProof = hashDomain("aloha/angstrom-v4/action-obligation-postcondition/v1", { rawActionHash: cutoffRaw.actionHash, routeBindingHash: route.routeBindingHash, exactEvaluationHash, obligationRoot, inputs: action.inputs, outputs: action.outputs, stateFactsRoot, poolKey, zeroForOne: true });
  ...
  void ignoredCutoff;
  reject({ ...cutoffBody, actionHash: hashDomain("aloha/angstrom-v4/search-action/v1", cutoffBody) });
});
