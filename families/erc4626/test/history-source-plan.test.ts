...
import {
  ...
  ERC4626_HISTORY_NOMINATION_PROGRAM,
  ERC4626_HISTORY_SOURCE_PLAN_RUNTIME,
  ERC4626_WITHDRAW_TOPIC,
} from "../src/public.ts";

const h = (value: string): Hash => hashDomain("test/erc4626-history", value);
...
const plan: SourcePlanRefV1 = Object.freeze({ ownerRef: h("owner"), sourcePlanRef: h("plan"), familyDefinitionHash: ERC4626_FAMILY_AUTHORING_HASH, completeness: "contiguous-history", historyStartBlock: "0" });
...
test("ERC4626 history scans the exact 10,000-block grid and nominates outside the empty recent window", async () => {
  const ranges: CanonicalJson[] = [];
  const result = await ERC4626_HISTORY_SOURCE_PLAN_RUNTIME.execute(
  ...
  const nominations = await ERC4626_HISTORY_NOMINATION_PROGRAM.evaluate({ execution: result.execution, sourceEvidence: result.sourceEvidence, recent: recent(cutoff), rawEvidence: rawPort(result) }, new AbortController().signal);
  assert.deepEqual(nominations.map(value => value.instanceNominationKey), [vault, address("8"), address("9")]);
});

test("ERC4626 history successor scans only the delta and retains predecessor inventory", async () => {
  const firstCutoff = Object.freeze({ chainId: "1", number: "99", hash: h("block-99"), stateRoot: h("state-99") });
  const first = await ERC4626_HISTORY_SOURCE_PLAN_RUNTIME.execute({ plan, cutoff: firstCutoff, previousAppliedThrough: null }, physical(() => [withdraw(95n)]), new AbortController().signal);
  const nextCutoff = Object.freeze({ chainId: "1", number: "100", hash: h("block-100"), stateRoot: h("state-100") });
  const ranges: CanonicalJson[] = [];
  const next = await ERC4626_HISTORY_SOURCE_PLAN_RUNTIME.execute(
  ...
  const nominations = await ERC4626_HISTORY_NOMINATION_PROGRAM.evaluate({ execution: next.execution, sourceEvidence: next.sourceEvidence, recent: recent(nextCutoff), rawEvidence: rawPort(next) }, new AbortController().signal);
  assert.deepEqual(nominations.map(value => value.instanceNominationKey), [vault]);
  await assert.rejects(() => ERC4626_HISTORY_SOURCE_PLAN_RUNTIME.execute({ plan, cutoff: nextCutoff, previousAppliedThrough: "99" }, physical(() => []), new AbortController().signal), /durable predecessor/);
});

test("ERC4626 history rejects physical binding, evidence, order and range mutations", async () => {
  await assert.rejects(() => ERC4626_HISTORY_SOURCE_PLAN_RUNTIME.execute({ plan, cutoff: { ...cutoff, number: "1" }, previousAppliedThrough: null }, physical(() => [], (observation, request) => { observation.request = { ...request.request, params: [{ fromBlock: "0x1", toBlock: "0x1", topics: [ERC4626_WITHDRAW_TOPIC] }] }; }), new AbortController().signal), /binding mismatch/);
  await assert.rejects(() => ERC4626_HISTORY_SOURCE_PLAN_RUNTIME.execute({ plan, cutoff: { ...cutoff, number: "1" }, previousAppliedThrough: null }, physical(() => [], undefined, h("forged-evidence")), new AbortController().signal), /binding mismatch/);
  await assert.rejects(() => ERC4626_HISTORY_SOURCE_PLAN_RUNTIME.execute({ plan, cutoff: { ...cutoff, number: "10" }, previousAppliedThrough: null }, physical(() => [withdraw(5n), withdraw(4n)]), new AbortController().signal), /strict chain order/);
  await assert.rejects(() => ERC4626_HISTORY_SOURCE_PLAN_RUNTIME.execute({ plan, cutoff: { ...cutoff, number: "10" }, previousAppliedThrough: null }, physical(() => [withdraw(11n)]), new AbortController().signal), /outside requested range/);
  ...
  await assert.rejects(() => ERC4626_HISTORY_SOURCE_PLAN_RUNTIME.execute({ plan, cutoff: { ...cutoff, number: "1" }, previousAppliedThrough: null }, forged, new AbortController().signal), /raw locator mismatch/);
});

test("ERC4626 history rejects a self-consistent missing-chunk splice", async () => {
  const result = await ERC4626_HISTORY_SOURCE_PLAN_RUNTIME.execute({ plan, cutoff, previousAppliedThrough: null }, physical(() => []), new AbortController().signal);
  const raw = new Map(result.rawEvidenceLocators.map(value => [value.rawLocatorHash, value.bytes]));
  ...
  await assert.rejects(() => ERC4626_HISTORY_NOMINATION_PROGRAM.evaluate({ execution, sourceEvidence, recent: recent(cutoff), rawEvidence: { read(hash) { return raw.get(hash)!; } } }, new AbortController().signal), /coverage gap/);
});
