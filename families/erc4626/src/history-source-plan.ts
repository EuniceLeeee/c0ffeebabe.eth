...
function decodeHistory(
  ...
  for (const chunk of chunks) {
    ...
    if (!sameCutoff(chunk.cutoff, execution.cutoff)) throw new TypeError("erc4626 history chunk cutoff mismatch");
  ...
  if (expectedFrom !== BigInt(execution.through) + 1n) throw new TypeError("erc4626 history chunk cutoff mismatch");
  const entries = chunks.flatMap(chunk => chunk.entries);
  const expected = { kind: "erc4626-withdraw-rolling-observation", version: 1, topic: ERC4626_WITHDRAW_TOPIC, from: execution.from, through: execution.through, chunkBlocks: CHUNK_BLOCKS.toString(), entryCount: String(entries.length) };
...
export const ERC4626_HISTORY_SOURCE_PLAN_RUNTIME: FamilySourcePlanRuntimeV1 = Object.freeze({
  ...
  async execute(input: FamilySourcePlanExecutionInputV1, physical: FamilySourcePlanPhysicalPortV1, signal: AbortSignal) {
    ...
    const opaqueResult: CanonicalJson = Object.freeze({ kind: "erc4626-withdraw-rolling-observation", version: 1, topic: ERC4626_WITHDRAW_TOPIC, from, through: input.cutoff.number, chunkBlocks: CHUNK_BLOCKS.toString(), entryCount: String(entries.length) });
