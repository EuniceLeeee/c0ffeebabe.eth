...
function exactResult(value: FamilySourcePlanPhysicalResultV1): FamilySourcePlanPhysicalResultV1 {
  if (value === null || typeof value !== "object" || Array.isArray(value) || Reflect.ownKeys(value).sort().join(",") !== "evidenceRef,rawEvidenceLocator,rawLocatorHash,response") throw new TypeError("erc4626 history physical result shape mismatch");
  if (!/^0x[0-9a-f]{64}$/.test(value.rawLocatorHash) || !/^0x[0-9a-f]{64}$/.test(value.evidenceRef)) throw new TypeError("erc4626 history physical result hash mismatch");
  const raw = value.rawEvidenceLocator;
  if (Reflect.ownKeys(raw).sort().join(",") !== "bytes,kind,rawLocatorHash,version" || raw.kind !== "raw-evidence-locator" || raw.version !== 1 || raw.rawLocatorHash !== value.rawLocatorHash || !(raw.bytes instanceof Uint8Array) || sha256Hex(raw.bytes) !== raw.rawLocatorHash) throw new TypeError("erc4626 history raw locator mismatch");
  return value;
}
...
function decodeRpcLogs(value: CanonicalJson, from: bigint, through: bigint): readonly RpcLogV1[] {
  if (!Array.isArray(value)) throw new TypeError("erc4626 history response must be a JSON-RPC log array");
  ...
  return Object.freeze(value.map((item, index): RpcLogV1 => {
    if (item === null || typeof item !== "object" || Array.isArray(item)) throw new TypeError(`erc4626 history log[${index}] must be an object`);
    const log = item as Record<string, CanonicalJson>;
    if (Object.keys(log).sort().join(",") !== "address,blockHash,blockNumber,data,logIndex,removed,topics,transactionHash,transactionIndex") throw new TypeError(`erc4626 history log[${index}] has an unexpected shape`);
    if (typeof log.address !== "string" || !/^0x[0-9a-f]{40}$/.test(log.address) || log.removed !== false || typeof log.data !== "string" || !/^0x(?:[0-9a-f]{2})*$/.test(log.data) || !Array.isArray(log.topics) || log.topics.length === 0) throw new TypeError(`erc4626 history log[${index}] is malformed`);
    const topics = Object.freeze(log.topics.map((topic, topicIndex) => hash(topic, `erc4626 history log[${index}].topics[${topicIndex}]`)));
    if (topics[0] !== ERC4626_WITHDRAW_TOPIC) throw new TypeError("erc4626 history topic mismatch");
    const block = quantity(log.blockNumber, `erc4626 history log[${index}].blockNumber`);
    const logIndex = quantity(log.logIndex, `erc4626 history log[${index}].logIndex`);
    quantity(log.transactionIndex, `erc4626 history log[${index}].transactionIndex`);
    if (block < from || block > through) throw new TypeError("erc4626 history log outside requested range");
    if (previous !== null && (block < previous.block || block === previous.block && logIndex <= previous.index)) throw new TypeError("erc4626 history logs are not in strict chain order");
    previous = { block, index: logIndex };
    const blockHash = hash(log.blockHash, `erc4626 history log[${index}].blockHash`);
    const transactionHash = hash(log.transactionHash, `erc4626 history log[${index}].transactionHash`);
    const key = `${blockHash}:${transactionHash}:${logIndex}`;
    if (seen.has(key)) throw new TypeError("erc4626 history contains a duplicate log");
    seen.add(key);
...
export function decodeErc4626WithdrawHistoryEntries(value: CanonicalJson, from: bigint, through: bigint): readonly Erc4626WithdrawHistoryEntryV1[] {
  return Object.freeze(decodeRpcLogs(value, from, through).flatMap((log): readonly Erc4626WithdrawHistoryEntryV1[] => {
...
function observe(value: FamilySourcePlanPhysicalResultV1, input: FamilySourcePlanExecutionInputV1, from: string, through: string) {
  ...
  const filter = { fromBlock: blockTag(from), toBlock: blockTag(through), topics: [ERC4626_WITHDRAW_TOPIC] };
  if (
  ...
  ) throw new TypeError("erc4626 history physical observation binding mismatch");
  return Object.freeze({ result, entries: decodeErc4626WithdrawHistoryEntries(result.response, BigInt(from), BigInt(through)) });
}
...
type HistoryChunkV1 = {
  ...
  readonly entries: readonly Erc4626WithdrawHistoryEntryV1[];
};
...
function decodeHistory(
  ...
  if (
    ...
    || execution.from !== (execution.previousAppliedThrough === null ? "0" : decimal(BigInt(execution.previousAppliedThrough) + 1n))
  ) throw new TypeError("erc4626 history execution/evidence mismatch");
  ...
  ) throw new TypeError("erc4626 history evidence index mismatch");

  const chunks = sourceEvidence.refs.map(evidence => {
    const bytes = rawEvidence.read(evidence.rawLocatorHash);
    if (sha256Hex(bytes) !== evidence.rawLocatorHash) throw new TypeError("erc4626 history raw hash mismatch");
    ...
    ) throw new TypeError("erc4626 history chunk range malformed");
    const range = rangeValue as { readonly from: string; readonly through: string };
    const from = quantity(blockTag(range.from), "erc4626 history chunk from");
    const through = quantity(blockTag(range.through), "erc4626 history chunk through");
    const filter = { fromBlock: blockTag(range.from), toBlock: blockTag(range.through), topics: [ERC4626_WITHDRAW_TOPIC] };
    if (
      observation.familyDefinitionHash !== ERC4626_FAMILY_AUTHORING_HASH
      ...
      || BigInt(observation.cutoff.number) > BigInt(execution.cutoff.number)
      || observation.requestSchemaHash !== ERC4626_HISTORY_SOURCE_PLAN_SCHEMA_HASH
    ...
    ) throw new TypeError("erc4626 history predecessor chunk binding mismatch");
    return Object.freeze({ evidence, from: range.from, through: range.through, cutoff: observation.cutoff, entries: decodeErc4626WithdrawHistoryEntries(observation.response, from, through) });
  }).sort((left, right) => BigInt(left.from) < BigInt(right.from) ? -1 : BigInt(left.from) > BigInt(right.from) ? 1 : 0);

  let expectedFrom = 0n;
  let epochCutoff: CanonicalCutoffV1 | null = null;
  for (const chunk of chunks) {
    if (BigInt(chunk.from) !== expectedFrom) throw new TypeError("erc4626 history chunk coverage gap");
    if (epochCutoff === null || !sameCutoff(chunk.cutoff, epochCutoff)) {
      if (epochCutoff !== null && expectedFrom !== BigInt(epochCutoff.number) + 1n) throw new TypeError("erc4626 history epoch splice");
      epochCutoff = chunk.cutoff;
    }
    const expectedThrough = expectedFrom + CHUNK_BLOCKS - 1n > BigInt(epochCutoff.number) ? BigInt(epochCutoff.number) : expectedFrom + CHUNK_BLOCKS - 1n;
    if (BigInt(chunk.through) !== expectedThrough) throw new TypeError("erc4626 history chunk grid mismatch");
    expectedFrom = expectedThrough + 1n;
  }
  if (epochCutoff === null || !sameCutoff(epochCutoff, execution.cutoff) || expectedFrom !== BigInt(execution.through) + 1n) throw new TypeError("erc4626 history chunk cutoff mismatch");
  const entries = chunks.flatMap(chunk => chunk.entries);
  const expected = { kind: "erc4626-withdraw-contiguous-history", version: 1, topic: ERC4626_WITHDRAW_TOPIC, from: "0", through: execution.through, chunkBlocks: CHUNK_BLOCKS.toString(), entries };
  if (encodeCanonicalJson(expected) !== encodeCanonicalJson(execution.opaqueResult)) throw new TypeError("erc4626 history result/raw mismatch");
  return Object.freeze(chunks);
}

export const ERC4626_HISTORY_SOURCE_PLAN_RUNTIME: FamilySourcePlanRuntimeV1 = Object.freeze({
  ...ERC4626_HISTORY_SOURCE_PLAN,
  async execute(input: FamilySourcePlanExecutionInputV1, physical: FamilySourcePlanPhysicalPortV1, signal: AbortSignal) {
    if (signal.aborted) throw signal.reason;
    if (input.plan.familyDefinitionHash !== ERC4626_FAMILY_AUTHORING_HASH || input.plan.completeness !== "contiguous-history" || input.plan.historyStartBlock !== "0") throw new TypeError("erc4626 history source plan binding mismatch");
    const predecessor = input.predecessor ?? null;
    if ((input.previousAppliedThrough === null) !== (predecessor === null)) throw new TypeError("erc4626 history durable predecessor mismatch");
    if (predecessor !== null && (
      !/^0x[0-9a-f]{64}$/.test(predecessor.persistedExecutionRoot)
    ...
    )) throw new TypeError("erc4626 history predecessor lineage mismatch");
    const predecessorChunks = predecessor === null
      ? Object.freeze([] as HistoryChunkV1[])
    ...
    for (let start = BigInt(from); start <= BigInt(input.cutoff.number); start += CHUNK_BLOCKS) {
      ...
      const filter = Object.freeze({ fromBlock: blockTag(range.from), toBlock: blockTag(range.through), topics: Object.freeze([ERC4626_WITHDRAW_TOPIC]) });
      const raw = await physical.request({ familyDefinitionHash: ERC4626_FAMILY_AUTHORING_HASH, plan: input.plan, cutoff: input.cutoff, requestSchemaHash: ERC4626_HISTORY_SOURCE_PLAN_SCHEMA_HASH, request: { kind: "family-source-plan-rpc", version: 1, method: "eth_getLogs", params: Object.freeze([filter]), target: null, manager: null, topic: ERC4626_WITHDRAW_TOPIC, lookback: range, chunk: Object.freeze({ maxBlocks: CHUNK_BLOCKS.toString() }) } }, signal);
      chunks.push({ ...range, ...observe(raw, input, range.from, range.through) });
    }
    const refs = Object.freeze([...predecessorChunks.map(chunk => chunk.evidence), ...chunks.map(chunk => evidenceRef(input, chunk.result))].sort((left, right) => refKey(left).localeCompare(refKey(right))));
    ...
    const rawEvidenceLocators = Object.freeze([...predecessorLocators, ...chunks.map(chunk => chunk.result.rawEvidenceLocator)].sort((left, right) => left.rawLocatorHash.localeCompare(right.rawLocatorHash)));
    ...
    const entries = Object.freeze([...predecessorChunks.flatMap(chunk => chunk.entries), ...chunks.flatMap(chunk => chunk.entries)]);
    ...
    const opaqueResult: CanonicalJson = Object.freeze({ kind: "erc4626-withdraw-contiguous-history", version: 1, topic: ERC4626_WITHDRAW_TOPIC, from: "0", through: input.cutoff.number, chunkBlocks: CHUNK_BLOCKS.toString(), entries: canonicalEntries });
    const resultPartitionRoot = hashDomain("aloha/erc4626/history-source-partition/v1", opaqueResult);
...
export const ERC4626_HISTORY_NOMINATION_PROGRAM: FamilySourcePlanNominationProgramV1 = Object.freeze({
  ...
  async evaluate(input: FamilySourcePlanNominationInputV1, signal: AbortSignal): Promise<readonly CandidateNominationV1[]> {
    ...
    if (
      input.execution.plan.familyDefinitionHash !== ERC4626_FAMILY_AUTHORING_HASH
    ...
    ) throw new TypeError("erc4626 history nomination binding mismatch");
    const chunks = decodeHistory(input.execution, input.sourceEvidence, input.rawEvidence);
    return Object.freeze(chunks.flatMap(chunk => [...new Set(chunk.entries.map(entry => entry.target))].sort().map(target => Object.freeze({ kind: "aloha.candidate-nomination" as const, version: "2" as const, familyId: ERC4626_FAMILY_ID, familyDefinitionHash: ERC4626_FAMILY_AUTHORING_HASH, instanceNominationKey: target, evidence: chunk.evidence }))));
  },
});
