...
import { ERC4626_FAMILY_ID, ERC4626_HISTORY_SOURCE_PLAN_ID, ERC4626_HISTORY_SOURCE_PLAN_SCHEMA_HASH, ERC4626_SOURCE_PLAN_ID, ERC4626_SOURCE_PLAN_SCHEMA_HASH } from "./manifest.ts";
import { ERC4626_FAMILY_AUTHORING_HASH } from "./family-definition.ts";

export const ERC4626_SOURCE_PLAN = defineFamilySourcePlan({
  sourcePlanId: ERC4626_SOURCE_PLAN_ID,
  completeness: "nomination-only",
  historyStartBlock: null,
  schemaHash: ERC4626_SOURCE_PLAN_SCHEMA_HASH,
});
...
function assertNominationBinding(input: FamilySourcePlanNominationInputV1): void {
  ...
  ) throw new TypeError("erc4626 nomination binding mismatch");
}
...
export const ERC4626_SOURCE_PLAN_RUNTIME: FamilySourcePlanRuntimeV1 = Object.freeze({
  ...ERC4626_SOURCE_PLAN,
  async execute(input: FamilySourcePlanExecutionInputV1, _physical: FamilySourcePlanPhysicalPortV1, signal: AbortSignal) {
    if (signal.aborted) throw signal.reason;
    if (input.plan.familyDefinitionHash !== ERC4626_FAMILY_AUTHORING_HASH || input.plan.completeness !== "nomination-only" || input.plan.historyStartBlock !== null || input.previousAppliedThrough !== null) {
      throw new TypeError("erc4626 source plan binding mismatch");
    }
    return sealPositiveOnlySourceExecution(input);
  },
});

export const ERC4626_SOURCE_NOMINATION_PROGRAM: FamilySourcePlanNominationProgramV1 = Object.freeze({
  ...
  async evaluate(input: FamilySourcePlanNominationInputV1, signal: AbortSignal): Promise<readonly CandidateNominationV1[]> {
    ...
    for (const evidence of input.recent.evidence) {
      ...
      const raw = decodeEvmLogObservationBytes(rawBytes, "erc4626.rawEvidence");
      if (
      ...
      ) throw new TypeError("erc4626 raw evidence/recent evidence mismatch");
      ...
      const seed = decodeErc4626Candidate(observation, "erc4626-call");
      if (seed === null) continue;
      const target = seed.target;
      if (seenTargets.has(target)) continue;
      seenTargets.add(target);
      const nomination = nominateErc4626({ target, evidence: observation });
      if (nomination.status !== "nominated") continue;
      nominations.push(Object.freeze({
        kind: "aloha.candidate-nomination" as const, version: "2" as const, familyId: ERC4626_FAMILY_ID, familyDefinitionHash: ERC4626_FAMILY_AUTHORING_HASH, instanceNominationKey: nomination.candidate.instanceNominationKey, evidence,
      }));
    }
