...
import { UNIV4_FAMILY_ID } from "../src/manifest.ts";
import { UNIV4_PHYSICAL_LIFECYCLE_ADAPTER_FACTORY } from "../src/runtime/physical-adapter.ts";

const h = (value: string): Hash => hashDomain("aloha/univ4-physical-test/v1", value);
...
test("UniV4 physical lifecycle reads exact qualified evidence and current virtual reserves", async () => {
  ...
  const adapter = UNIV4_PHYSICAL_LIFECYCLE_ADAPTER_FACTORY();
...
test("UniV4 physical lifecycle preserves transport failure and rejects unqualified evidence", async () => {
