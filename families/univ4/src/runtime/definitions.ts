...
const STAGE_IDS = Object.freeze({
  nomination: `family.${UNIV4_FAMILY_ID}.nomination`,
  identity: `family.${UNIV4_FAMILY_ID}.identity`,
  materialization: `family.${UNIV4_FAMILY_ID}.materialization`,
  projection: `family.${UNIV4_FAMILY_ID}.projection`,
  rehydration: `family.${UNIV4_FAMILY_ID}.rehydration`,
});

const STAGE_SCHEMA_HASHES = Object.freeze({
  nomination: hashDomain("aloha/univ4/stage-schema/v1", "nomination"),
  identity: hashDomain("aloha/univ4/stage-schema/v1", "identity"),
  materialization: hashDomain("aloha/univ4/stage-schema/v1", "materialization"),
  projection: hashDomain("aloha/univ4/stage-schema/v1", "projection"),
  rehydration: hashDomain("aloha/univ4/stage-schema/v1", "rehydration"),
});

const REQUESTED_ARTIFACT_DEPENDENCY_ROOT = hashDomain(
  "aloha/univ4/requested-artifact-dependencies/v1",
  { familyId: UNIV4_FAMILY_ID, version: UNIV4_FAMILY_VERSION },
);

interface Univ4CandidateRecordV1 {
  readonly kind: "aloha.candidate-record";
  readonly version: "2";
  readonly familyId: typeof UNIV4_FAMILY_ID;
...
type CandidateBindingV1 = Univ4CandidateRecordV1;

interface IdentityMemoV1 {
  readonly kind: "univ4-identity-memo";
  readonly familyId: typeof UNIV4_FAMILY_ID;
  readonly familyDefinitionHash: Hash;
  readonly familyCandidateKey: Hash;
  readonly instanceNominationKey: string;
...
interface NominationPayloadV1 {
  readonly kind: "univ4-nomination-input";
...
interface IdentityPayloadV1 {
  readonly kind: "univ4-identity-input";
  readonly binding: CandidateBindingV1;
  readonly cutoff: Univ4CutoffV1;
  readonly readPlan: readonly string[];
...
interface MaterializationPayloadV1 {
  readonly kind: "univ4-materialization-input";
  readonly binding: CandidateBindingV1;
...
interface MaterializationOutputV1 {
  readonly kind: "univ4-materialization-output";
  readonly binding: CandidateBindingV1;
  readonly identityMemoHash: Hash;
  readonly identityFactsHash: Hash;
  readonly cutoff: Univ4CutoffV1;
  readonly state: Univ4MaterializedStateV1;
}

interface ProjectionPayloadV1 {
  readonly kind: "univ4-projection-input";
  readonly binding: CandidateBindingV1;
  readonly identityMemo: IdentityMemoV1;
...
interface RehydrationPayloadV1 {
  readonly kind: "univ4-rehydration-input";
  readonly binding: CandidateBindingV1;
  readonly cutoff: Univ4CutoffV1;
  readonly readPlan: readonly string[];
  readonly requestId: Hash;
...
interface NominationOutputV1 {
  readonly kind: "univ4-nomination-verified";
  readonly binding: CandidateBindingV1;
  readonly cutoff: Univ4CutoffV1;
  readonly requestId: Hash;
}
...
interface RehydrationOutputV1 {
  readonly kind: "univ4-rehydration-verified";
  readonly binding: CandidateBindingV1;
  readonly cutoff: Univ4CutoffV1;
...
function cutoff(value: unknown, path: string): Univ4CutoffV1 {
  return decodeExactObject<Univ4CutoffV1>(value, {
...
function sameCutoff(left: Univ4CutoffV1, right: Univ4CutoffV1): boolean {
...
function observation(value: unknown, path: string): Univ4ObservationV1 {
  const decoded = decodeExactObject<Univ4ObservationV1>(value, {
...
function candidateRecord(value: unknown, path: string): Univ4CandidateRecordV1 {
  const decoded = decodeExactObject<Univ4CandidateRecordV1>(value, {
    ...
    familyId: (item, itemPath) => { if (item !== UNIV4_FAMILY_ID) throw new TypeError(`${itemPath} family mismatch`); return UNIV4_FAMILY_ID; },
  ...
  if (decoded.familyDefinitionHash !== UNIV4_FAMILY_DEFINITION_HASH) throw new TypeError("univ4-candidate-definition-mismatch");
  if (decoded.familyCandidateKey !== discoveryFamilyCandidateKey(decoded.familyDefinitionHash, decoded.instanceNominationKey)) {
    throw new TypeError("univ4-candidate-key-mismatch");
  }
  if (decoded.candidateSubjectHash !== candidateSubjectHash(decoded.familyDefinitionHash, decoded.instanceNominationKey)) throw new TypeError("univ4-candidate-subject-mismatch");
  if (decoded.evidence.length === 0) throw new TypeError("univ4-candidate-evidence-empty");
  const evidenceKeys = decoded.evidence.map(item => hashDomain("aloha/candidate-evidence-ref/v1", item));
  if (new Set(evidenceKeys).size !== evidenceKeys.length) throw new TypeError("univ4-candidate-evidence-duplicate");
  const sortedEvidenceKeys = [...evidenceKeys].sort();
  if (evidenceKeys.some((item, index) => item !== sortedEvidenceKeys[index])) throw new TypeError("univ4-candidate-evidence-order");
  if (decoded.candidateEvidenceRoot !== candidateEvidenceRoot(decoded.evidence)) throw new TypeError("univ4-candidate-evidence-root-mismatch");
  for (const evidence of decoded.evidence) {
    if (evidence.kind === "recent-log" && (address(evidence.address, `${path}.evidence.address`) !== canonicalAddress(UNIV4_POOL_MANAGER) || evidence.topic !== UNIV4_CONTRACT_EVIDENCE_TOPIC)) {
      throw new TypeError("univ4-candidate-recent-evidence-pattern-mismatch");
    }
  }
  return deepFreeze(decoded);
}

function candidateBindingFromRecord(value: unknown, cutoffValue: Univ4CutoffV1, path: string): CandidateBindingV1 {
  ...
  for (const evidence of record.evidence) {
    ...
    if (block > through) throw new TypeError("univ4-candidate-evidence-after-cutoff");
    if (block < from) throw new TypeError("univ4-candidate-evidence-before-window");
  }
  return record;
}

function assertCandidateBinding(binding: CandidateBindingV1, cutoffValue: Univ4CutoffV1): void {
  cutoff(cutoffValue, "univ4.binding.cutoff");
  candidateRecord(binding, "univ4.payload.candidate");
}
...
function primaryEvidence(binding: CandidateBindingV1): CandidateEvidenceRefV1 {
  ...
  if (evidence === undefined) throw new TypeError("univ4 candidate evidence is empty");
  return evidence;
}

function identityRequestId(binding: CandidateBindingV1, cutoffValue: Univ4CutoffV1): Hash {
  const evidence = primaryEvidence(binding);
  return hashDomain("aloha/univ4/request-id/v1", {
    phase: "identity",
...
function stateRequestId(instanceKey: string, cutoffValue: Univ4CutoffV1): Hash {
  return hashDomain("aloha/univ4/request-id/v1", { phase: "materialization", target: instanceKey, operation: "state", cutoff: cutoffValue });
}

function nominationRequestId(binding: CandidateBindingV1, cutoffValue: Univ4CutoffV1): Hash {
  return hashDomain("aloha/univ4/request-id/v1", {
    phase: "nomination",
...
function rehydrationReferenceHash(binding: CandidateBindingV1, cutoffValue: Univ4CutoffV1): Hash {
  return hashDomain("aloha/univ4/rehydration-reference/v1", {
...
function rehydrationRequestId(binding: CandidateBindingV1, cutoffValue: Univ4CutoffV1): Hash {
  return hashDomain("aloha/univ4/request-id/v1", {
    phase: "rehydration",
    target: UNIV4_POOL_MANAGER,
...
function identityMemo(value: unknown, path: string): IdentityMemoV1 {
  const decoded = decodeExactObject<IdentityMemoV1>(value, {
    kind: (item, itemPath) => { if (item !== "univ4-identity-memo") throw new TypeError(`${itemPath} kind mismatch`); return "univ4-identity-memo"; },
    familyId: (item, itemPath) => { if (item !== UNIV4_FAMILY_ID) throw new TypeError(`${itemPath} family mismatch`); return UNIV4_FAMILY_ID; },
    familyDefinitionHash: (item, itemPath) => assertHash(item, itemPath),
    familyCandidateKey: (item, itemPath) => assertHash(item, itemPath),
    instanceNominationKey: (item, itemPath) => instanceKey(item, itemPath),
  ...
  if (decoded.familyDefinitionHash !== UNIV4_FAMILY_DEFINITION_HASH) throw new TypeError("univ4 identity definition mismatch");
  if (decoded.instanceNominationKey !== decoded.identity.instanceKey) throw new TypeError("univ4 identity nomination key mismatch");
  if (decoded.candidateSnapshotHash !== decoded.identity.candidateSnapshotHash) throw new TypeError("univ4 identity snapshot mismatch");
  if (decoded.candidateSnapshotHash !== candidateSubjectHash(decoded.familyDefinitionHash, decoded.instanceNominationKey)) throw new TypeError("univ4 identity subject mismatch");
  if (decoded.familyCandidateKey !== discoveryFamilyCandidateKey(decoded.familyDefinitionHash, decoded.instanceNominationKey)) throw new TypeError("univ4 identity candidate key mismatch");
  return decoded;
}

function decodeManagerBinding(value: unknown, path: string): NonNullable<Univ4IdentityV1["facts"]["managerBinding"]> {
  ...
  return decodeExactObject<NonNullable<Univ4IdentityV1["facts"]["managerBinding"]>>(raw, {
...
function decodeIdentityFacts(value: unknown, path: string): Univ4IdentityV1["facts"] {
  const raw = canonical(value);
  if (raw === null || typeof raw !== "object" || Array.isArray(raw)) throw new TypeError(`${path} must be an identity facts object`);
  return decodeExactObject<Univ4IdentityV1["facts"]>(raw, {
    target: (item, itemPath) => address(item, itemPath),
...
function decodeIdentity(value: unknown, path: string): Univ4IdentityV1 {
  const decoded = decodeExactObject<Univ4IdentityV1>(value, {
    cutoff: (item, itemPath) => cutoff(item, itemPath),
    candidateSnapshotHash: (item, itemPath) => assertHash(item, itemPath),
  ...
  if (
    ...
    || decoded.facts.target !== canonicalAddress(UNIV4_POOL_MANAGER)
    || managerBinding.manager !== decoded.facts.target
    || managerBinding.stateView !== canonicalAddress(UNIV4_STATE_VIEW)
    || managerBinding.quoter !== canonicalAddress(UNIV4_QUOTER)
  ...
  ) throw new TypeError("univ4 identity strict PoolKey binding mismatch");
  if (decoded.factsHash !== hashDomain("aloha/univ4/identity-facts/v1", decoded.facts)) throw new TypeError("univ4 identity facts hash mismatch");
  return decoded;
}
...
function assertIdentityBinding(binding: CandidateBindingV1, memo: IdentityMemoV1, cutoffValue: Univ4CutoffV1): void {
  ...
  ) throw new TypeError("univ4 identity lineage mismatch");
}

function decodeState(value: unknown, path: string): Univ4MaterializedStateV1 {
  const decoded = decodeExactObject<Univ4MaterializedStateV1>(value, {
    cutoff: (item, itemPath) => cutoff(item, itemPath),
    instanceKey: (item, itemPath) => instanceKey(item, itemPath),
  ...
  if (decoded.stateHash !== resealUniv4State(decoded)) throw new TypeError("univ4 materialized state hash mismatch");
  return decoded;
}
...
function decodeMaterializationOutput(value: unknown, path: string): MaterializationOutputV1 {
  const decoded = decodeExactObject<MaterializationOutputV1>(value, {
    kind: (item, itemPath) => { if (item !== "univ4-materialization-output") throw new TypeError(`${itemPath} kind mismatch`); return "univ4-materialization-output"; },
  ...
  ) throw new TypeError("univ4 materialization output lineage mismatch");
  return decoded;
}
...
function decodeNominationPayload(value: unknown, path = "univ4.nominationPayload"): NominationPayloadV1 {
  const decoded = decodeExactObject<NominationPayloadV1>(value, {
    kind: (item, itemPath) => { if (item !== "univ4-nomination-input") throw new TypeError(`${itemPath} kind mismatch`); return "univ4-nomination-input"; },
    binding: (item, itemPath) => decodeCandidateBinding(item, itemPath),
    cutoff: (item, itemPath) => cutoff(item, itemPath),
  ...
  assertCandidateBinding(decoded.binding, decoded.cutoff);
  if (decoded.requestId !== nominationRequestId(decoded.binding, decoded.cutoff)) throw new TypeError("univ4 nomination request mismatch");
  return decoded;
}

function decodeIdentityPayload(value: unknown, path = "univ4.identityPayload"): IdentityPayloadV1 {
  const decoded = decodeExactObject<IdentityPayloadV1>(value, {
    kind: (item, itemPath) => { if (item !== "univ4-identity-input") throw new TypeError(`${itemPath} kind mismatch`); return "univ4-identity-input"; },
    binding: (item, itemPath) => decodeCandidateBinding(item, itemPath),
    cutoff: (item, itemPath) => cutoff(item, itemPath),
  ...
  if (decoded.requestIds.length !== expectedIds.length || decoded.requestIds.some((item, index) => item !== expectedIds[index])) throw new TypeError("univ4 identity request mismatch");
  return decoded;
}

function decodeMaterializationPayload(value: unknown, path = "univ4.materializationPayload"): MaterializationPayloadV1 {
  const decoded = decodeExactObject<MaterializationPayloadV1>(value, {
    kind: (item, itemPath) => { if (item !== "univ4-materialization-input") throw new TypeError(`${itemPath} kind mismatch`); return "univ4-materialization-input"; },
    binding: (item, itemPath) => decodeCandidateBinding(item, itemPath),
  ...
  if (decoded.requestId !== stateRequestId(decoded.identityMemo.identity.instanceKey, decoded.cutoff)) throw new TypeError("univ4 materialization request mismatch");
  return decoded;
}

function decodeProjectionPayload(value: unknown, path = "univ4.projectionPayload"): ProjectionPayloadV1 {
  const decoded = decodeExactObject<ProjectionPayloadV1>(value, {
    kind: (item, itemPath) => { if (item !== "univ4-projection-input") throw new TypeError(`${itemPath} kind mismatch`); return "univ4-projection-input"; },
    binding: (item, itemPath) => decodeCandidateBinding(item, itemPath),
    identityMemo: (item, itemPath) => identityMemo(item, itemPath),
  ...
  ) throw new TypeError("univ4 projection materialization lineage mismatch");
  if (decoded.requestId !== stateRequestId(decoded.identityMemo.identity.instanceKey, decoded.cutoff)) throw new TypeError("univ4 projection request mismatch");
  return decoded;
}

function decodeRehydrationPayload(value: unknown, path = "univ4.rehydrationPayload"): RehydrationPayloadV1 {
  const decoded = decodeExactObject<RehydrationPayloadV1>(value, {
    kind: (item, itemPath) => { if (item !== "univ4-rehydration-input") throw new TypeError(`${itemPath} kind mismatch`); return "univ4-rehydration-input"; },
    binding: (item, itemPath) => decodeCandidateBinding(item, itemPath),
    cutoff: (item, itemPath) => cutoff(item, itemPath),
  ...
  assertCandidateBinding(decoded.binding, decoded.cutoff);
  if (decoded.referenceHash !== rehydrationReferenceHash(decoded.binding, decoded.cutoff)) throw new TypeError("univ4 rehydration reference mismatch");
  if (decoded.requestId !== rehydrationRequestId(decoded.binding, decoded.cutoff)) throw new TypeError("univ4 rehydration request mismatch");
  return decoded;
}

function decodeIdentityObservation(value: unknown, path = "univ4.identityOutput"): IdentityObservationV1 {
  ...
  if (decoded.familyInstanceKey !== decoded.identityMemo.identity.instanceKey) throw new TypeError("univ4 identity output instance mismatch");
  if (decoded.identityMemoHash !== identityMemoHash(decoded.identityMemo)) throw new TypeError("univ4 identity output memo hash mismatch");
  if (decoded.descriptorHash !== identityDescriptorHash(decoded.identityMemo.identity)) throw new TypeError("univ4 identity output descriptor mismatch");
  if (decoded.evidenceRoot !== decoded.identityMemo.candidateEvidenceRoot) throw new TypeError("univ4 identity output evidence mismatch");
  return decoded;
}

function decodeNominationOutput(value: unknown, path = "univ4.nominationOutput"): NominationOutputV1 {
  const decoded = decodeExactObject<NominationOutputV1>(value, {
    kind: (item, itemPath) => { if (item !== "univ4-nomination-verified") throw new TypeError(`${itemPath} kind mismatch`); return "univ4-nomination-verified"; },
    binding: (item, itemPath) => decodeCandidateBinding(item, itemPath),
    cutoff: (item, itemPath) => cutoff(item, itemPath),
    requestId: (item, itemPath) => assertHash(item, itemPath),
  }, path);
  assertCandidateBinding(decoded.binding, decoded.cutoff);
  if (decoded.requestId !== nominationRequestId(decoded.binding, decoded.cutoff)) throw new TypeError("univ4 nomination output request mismatch");
  return decoded;
}

function decodeRehydrationOutput(value: unknown, path = "univ4.rehydrationOutput"): RehydrationOutputV1 {
  const decoded = decodeExactObject<RehydrationOutputV1>(value, {
    kind: (item, itemPath) => { if (item !== "univ4-rehydration-verified") throw new TypeError(`${itemPath} kind mismatch`); return "univ4-rehydration-verified"; },
    binding: (item, itemPath) => decodeCandidateBinding(item, itemPath),
    cutoff: (item, itemPath) => cutoff(item, itemPath),
    instanceKey: (item, itemPath) => instanceKey(item, itemPath),
    referenceHash: (item, itemPath) => assertHash(item, itemPath),
  ...
  assertCandidateBinding(decoded.binding, decoded.cutoff);
  if (decoded.instanceKey !== decoded.binding.instanceNominationKey) throw new TypeError("univ4 rehydration instance mismatch");
  if (decoded.referenceHash !== rehydrationReferenceHash(decoded.binding, decoded.cutoff)) throw new TypeError("univ4 rehydration output reference mismatch");
  if (decoded.requestId !== rehydrationRequestId(decoded.binding, decoded.cutoff)) throw new TypeError("univ4 rehydration output request mismatch");
  return decoded;
}
...
function boundFacts(program: FrozenProgramEnvelopeV1, facts: readonly TransportFactV1[], ids: readonly Hash[]): readonly TransportFactV1[] {
  const decoded = fieldArray(facts, (item, itemPath) => decodeTransportFact(item, itemPath), "univ4.transportFacts");
  if (decoded.length !== ids.length || new Set(decoded.map(item => item.requestId)).size !== decoded.length) throw new TypeError("univ4 transport request partition mismatch");
  const programSource = source(program.source, "univ4.program.source");
  ...
  return Object.freeze(ids.map((id, index) => {
    ...
    if (fact === undefined) throw new TypeError(`univ4 missing request fact ${index}`);
    if (fact.requestFingerprint !== program.requestFingerprint || !sameSource(fact.source, programSource)) throw new TypeError("univ4 transport fact source mismatch");
    return fact;
  }));
}
...
function assertProgramForStage(
  ...
  if (input.dependencyRefs.length !== 0) throw new TypeError("univ4 lifecycle dependency closure is not empty");
  const programSource = source(input.program.source, "univ4.program.source");
  if (
  ...
  ) throw new TypeError(`univ4 ${stage} program binding mismatch`);
  if (!Object.isFrozen(programSource)) throw new TypeError("univ4 program source must be frozen");
}
...
function candidateFromRecentEvidence(
  ...
  cutoffValue: Univ4CutoffV1,
  evidence: Extract<CandidateEvidenceRefV1, { readonly kind: "recent-log" }>,
  rawBytes: Uint8Array,
): { readonly candidate: Univ4CandidateV1; readonly poolKey: ReturnType<typeof assertPoolKey> } {
  const raw = decodeEvmLogObservationBytes(rawBytes, "univ4.identity.initializeEvidence");
  if (
  ...
  ) throw new TypeError("univ4 identity recent evidence mismatch");
  const initialize = decodeUniv4InitializeLog(raw, UNIV4_CONTRACT_EVIDENCE_TOPIC);
  const decoded = decodeUniv4Candidate({
    kind: "log",
    cutoff: cutoffValue,
  ...
  if (decoded === null) throw new TypeError("univ4 recent evidence pattern mismatch");
  const nomination = nominateUniv4({ ...decoded, poolId: initialize.poolId });
  if (nomination.status !== "nominated" || nomination.candidate.instanceNominationKey !== binding.instanceNominationKey) {
    throw new TypeError("univ4 recent evidence candidate mismatch");
  }
...
function sourcePlanLogCandidate(
  ...
  if (
    observed.familyDefinitionHash !== UNIV4_FAMILY_DEFINITION_HASH
    ...
    || observed.request.target !== canonicalAddress(UNIV4_POOL_MANAGER)
    || observed.request.manager !== canonicalAddress(UNIV4_POOL_MANAGER)
    || observed.request.topic !== UNIV4_CONTRACT_EVIDENCE_TOPIC
  ) throw new TypeError("univ4 history evidence binding mismatch");
  ...
  const matches: { readonly candidate: Univ4CandidateV1; readonly poolKey: ReturnType<typeof assertPoolKey> }[] = [];
  for (const [index, value] of observed.response.entries()) {
    if (value === null || typeof value !== "object" || Array.isArray(value)) throw new TypeError(`univ4 history log[${index}] malformed`);
    ...
    if (
      ...
      || log.address !== canonicalAddress(UNIV4_POOL_MANAGER)
      || log.removed !== false
    ...
    ) throw new TypeError(`univ4 history log[${index}] malformed`);
    const topics = log.topics as readonly Hash[];
    if (topics[0] !== UNIV4_CONTRACT_EVIDENCE_TOPIC) continue;
    const initialize = decodeUniv4InitializeLog({ address: log.address, topics, data: log.data }, UNIV4_CONTRACT_EVIDENCE_TOPIC);
    if (initialize.poolId !== binding.instanceNominationKey) continue;
    const blockNumber = rpcQuantity(log.blockNumber, `univ4 history log[${index}].blockNumber`);
    if (BigInt(blockNumber) < BigInt(range.from) || BigInt(blockNumber) > BigInt(range.through)) throw new TypeError("univ4 history log outside observed range");
    const evmLog = Object.freeze({
      kind: "evm-log" as const,
      version: 1 as const,
      blockNumber,
      blockHash: assertHash(log.blockHash, `univ4 history log[${index}].blockHash`),
      transactionHash: assertHash(log.transactionHash, `univ4 history log[${index}].transactionHash`),
      logIndex: rpcQuantity(log.logIndex, `univ4 history log[${index}].logIndex`),
      address: canonicalAddress(log.address),
      topics: Object.freeze([...topics]),
      data: log.data,
    });
    rpcQuantity(log.transactionIndex, `univ4 history log[${index}].transactionIndex`);
    const logRawLocatorHash = sha256Hex(encodeEvmLogObservation(evmLog));
    const decoded = decodeUniv4Candidate({ kind: "log", cutoff: cutoffValue, blockNumber: evmLog.blockNumber, blockHash: evmLog.blockHash, txHash: evmLog.transactionHash, logIndex: evmLog.logIndex, target: evmLog.address, topic: UNIV4_CONTRACT_EVIDENCE_TOPIC, rawLocatorHash: logRawLocatorHash }, "univ4-contract-log");
    if (decoded === null) throw new TypeError("univ4 history evidence pattern mismatch");
    const nomination = nominateUniv4({ ...decoded, poolId: initialize.poolId });
    if (nomination.status !== "nominated") throw new TypeError("univ4 history candidate rejected");
    matches.push(Object.freeze({ candidate: nomination.candidate, poolKey: initialize.poolKey }));
  }
  if (matches.length !== 1) throw new TypeError("univ4 history evidence must contain exactly one candidate log");
  return matches[0]!;
}

function candidateFromIdentityEvidence(binding: CandidateBindingV1, cutoffValue: Univ4CutoffV1, rawBytes: Uint8Array): { readonly candidate: Univ4CandidateV1; readonly poolKey: ReturnType<typeof assertPoolKey> } {
  const evidence = primaryEvidence(binding);
  if (sha256Hex(rawBytes) !== evidence.rawLocatorHash) throw new TypeError("univ4 identity raw locator mismatch");
...
function portRef(instanceKey: string, direction: "input" | "output", asset: string): Hash {
  return hashDomain("aloha/univ4/asset-port/v1", { instanceKey, direction, asset });
}
...
function transition(
  ...
  state: Univ4MaterializedStateV1,
  ...
  const opaqueTransitionRef = hashDomain("aloha/univ4/transition/v1", {
    instanceKey,
    inputAsset,
    outputAsset,
    routeBindingHash,
...
function publication(
  ...
  const routeList = deriveUniv4Routes(payload.identityMemo.identity);
  const route = routeList[0];
  if (route === undefined) throw new TypeError("univ4 route derivation returned no route");
  ...
  return sealInstancePublication({
    ...
    staticProjectionMemoHash: hashDomain("aloha/univ4/static-projection-memo/v1", transitions),
    requestedArtifactDependencyRoot: REQUESTED_ARTIFACT_DEPENDENCY_ROOT,
    validityDependencyRoot: hashDomain("aloha/univ4/validity-dependencies/v1", {
...
function decodePublication(value: unknown, path = "univ4.projectionOutput"): InstancePublicationV1 {
  ...
  if (decoded.familyDefinitionHash !== UNIV4_FAMILY_DEFINITION_HASH) throw new TypeError("univ4 publication definition mismatch");
  ...
  ) throw new TypeError("univ4 publication lineage mismatch");
  const routeList = deriveUniv4Routes(memo.identity);
  const route = routeList[0];
  const sealedTransition = decoded.transitions[0];
  if (route === undefined || sealedTransition === undefined) throw new TypeError("univ4 publication route missing");
  const expectedDraft = transitionDraft(sealedTransition);
  if (decoded.staticProjectionMemoHash !== hashDomain("aloha/univ4/static-projection-memo/v1", [expectedDraft])) throw new TypeError("univ4 publication projection memo mismatch");
  const inputPort = sealedTransition.inputAssetPorts[0];
  const outputPort = sealedTransition.outputAssetPorts[0];
  if (sealedTransition.inputAssetPorts.length !== 1 || sealedTransition.outputAssetPorts.length !== 1 || sealedTransition.constraintRefs.length !== 1) throw new TypeError("univ4 publication transition shape mismatch");
  const stateHash = sealedTransition.constraintRefs[0];
  if (stateHash === undefined) throw new TypeError("univ4 publication state constraint missing");
  if (sealedTransition.opaqueTransitionRef !== hashDomain("aloha/univ4/transition/v1", {
    instanceKey: route.instanceKey,
    inputAsset: route.inputAsset,
    outputAsset: route.outputAsset,
    routeBindingHash: route.routeBindingHash,
    stateHash,
  })) throw new TypeError("univ4 publication transition binding mismatch");
  if (decoded.validityDependencyRoot !== hashDomain("aloha/univ4/validity-dependencies/v1", {
    identityFactsHash: memo.identity.factsHash,
    stateHash,
  })) throw new TypeError("univ4 publication validity binding mismatch");
  if (
    inputPort === undefined
  ...
  ) throw new TypeError("univ4 publication asset binding mismatch");
...
function identityOutput(value: unknown): CanonicalJson {
  return outputObject(decodeIdentityObservation(value), "univ4.identityOutput");
}

function materializationOutput(value: unknown): CanonicalJson {
  return outputObject(decodeMaterializationOutput(value, "univ4.materializationOutput"), "univ4.materializationOutput");
}

function projectionOutput(value: unknown): CanonicalJson {
  return outputObject(decodePublication(value), "univ4.projectionOutput");
}

function nominationOutput(value: unknown): CanonicalJson {
  return outputObject(decodeNominationOutput(value), "univ4.nominationOutput");
}

function rehydrationOutput(value: unknown): CanonicalJson {
  return outputObject(decodeRehydrationOutput(value), "univ4.rehydrationOutput");
}

function requireStage(input: FamilyStageGenericInvocationV1, expected: FamilyRuntimeStageV1): void {
  if (input.stage !== expected) throw new TypeError(`univ4-${expected}-stage-mismatch`);
}

function requireNoPriorOutputs(input: FamilyStageGenericInvocationV1): void {
  if (input.identityMemo !== null || input.materializationOutput !== null) throw new TypeError("univ4-unexpected-prior-stage-output");
}
...
function prepareNomination(input: FamilyStageGenericInvocationV1): NominationPayloadV1 {
  ...
  const cutoffValue = cutoff(input.cutoff, "univ4.nominationInvocation.cutoff");
  const binding = candidateBindingFromRecord(input.candidate, cutoffValue, "univ4.nominationInvocation.candidate");
  return Object.freeze({
    kind: "univ4-nomination-input",
    binding,
    cutoff: cutoffValue,
...
function prepareIdentity(input: FamilyStageGenericInvocationV1): IdentityPayloadV1 {
  ...
  requireNoPriorOutputs(input);
  const cutoffValue = cutoff(input.cutoff, "univ4.identityInvocation.cutoff");
  const binding = candidateBindingFromRecord(input.candidate, cutoffValue, "univ4.identityInvocation.candidate");
  return Object.freeze({
    kind: "univ4-identity-input",
    binding,
    cutoff: cutoffValue,
...
function prepareMaterialization(input: FamilyStageGenericInvocationV1): MaterializationPayloadV1 {
  ...
  if (input.identityMemo === null || input.materializationOutput !== null) throw new TypeError("univ4-materialization-prior-output-mismatch");
  const cutoffValue = cutoff(input.cutoff, "univ4.materializationInvocation.cutoff");
  const binding = candidateBindingFromRecord(input.candidate, cutoffValue, "univ4.materializationInvocation.candidate");
  const memo = identityMemo(input.identityMemo, "univ4.materializationInvocation.identityMemo");
  assertIdentityBinding(binding, memo, cutoffValue);
  return Object.freeze({
    kind: "univ4-materialization-input",
    binding,
    identityMemo: memo,
    cutoff: cutoffValue,
...
function prepareProjection(input: FamilyStageGenericInvocationV1): ProjectionPayloadV1 {
  ...
  if (input.identityMemo === null || input.materializationOutput === null) throw new TypeError("univ4-projection-prior-output-missing");
  const cutoffValue = cutoff(input.cutoff, "univ4.projectionInvocation.cutoff");
  const binding = candidateBindingFromRecord(input.candidate, cutoffValue, "univ4.projectionInvocation.candidate");
  const memo = identityMemo(input.identityMemo, "univ4.projectionInvocation.identityMemo");
  assertIdentityBinding(binding, memo, cutoffValue);
  const materialization = decodeMaterializationOutput(input.materializationOutput, "univ4.projectionInvocation.materializationOutput");
  if (
...
function prepareRehydration(input: FamilyStageGenericInvocationV1): RehydrationPayloadV1 {
  ...
  requireNoPriorOutputs(input);
  const cutoffValue = cutoff(input.cutoff, "univ4.rehydrationInvocation.cutoff");
  const binding = candidateBindingFromRecord(input.candidate, cutoffValue, "univ4.rehydrationInvocation.candidate");
  return Object.freeze({
    kind: "univ4-rehydration-input",
    binding,
    cutoff: cutoffValue,
...
function interpretNomination(input: StageInterpretInputV1): ProgramInterpretationDraftV1 {
  try {
    ...
    const fact = returned(facts[0]!, "univ4.nomination");
    if (fact.dataHex !== ackData(payload.binding.candidateSubjectHash)) throw new TypeError("univ4-nomination-ack-mismatch");
    return Object.freeze({ kind: "verified", output: Object.freeze({
      kind: "univ4-nomination-verified",
      binding: payload.binding,
      cutoff: payload.cutoff,
      requestId: payload.requestId,
    }) });
  } catch (error) {
    return Object.freeze({ kind: "invalidProgram", code: invalidCode(error, "univ4-nomination-invalid") });
  }
}
...
function interpretIdentity(input: StageInterpretInputV1): ProgramInterpretationDraftV1 {
  try {
    ...
    const rawBytes = hexBytes(returned(facts[0]!, "univ4.identity.initializeEvidence").dataHex, "univ4.identity.initializeEvidence");
    const resolved = candidateFromIdentityEvidence(payload.binding, payload.cutoff, rawBytes);
    const candidate = resolved.candidate;
    const result = verifyUniv4IdentityStage({
      candidate,
      reads: {
        cutoff: payload.cutoff,
        ...
        managerBinding: Object.freeze({ manager: UNIV4_POOL_MANAGER, stateView: UNIV4_STATE_VIEW, quoter: UNIV4_QUOTER }),
      },
    });
    ...
    return Object.freeze({ kind: "invalidProgram", code: invalidCode(error, "univ4-identity-invalid") });
  }
}
...
function interpretMaterialization(input: StageInterpretInputV1): ProgramInterpretationDraftV1 {
  try {
    ...
    const facts = boundFacts(input.program, input.facts, [payload.requestId]);
    const fact = returned(facts[0]!, "univ4.materialization.state");
    const reserves = decodeReserveWords(fact.dataHex, "univ4.materialization.state");
    const result = materializeUniv4({
    ...
    if (result.status === "chain-proven-rejected") return Object.freeze({ kind: "chainProvenRejected", factSet: input.factSet, decisionCode: result.reasonCode });
    return Object.freeze({ kind: "verified", output: Object.freeze({
      kind: "univ4-materialization-output",
      binding: payload.binding,
      identityMemoHash: identityMemoHash(payload.identityMemo),
      identityFactsHash: payload.identityMemo.identity.factsHash,
      cutoff: payload.cutoff,
    ...
    return Object.freeze({ kind: "invalidProgram", code: invalidCode(error, "univ4-materialization-invalid") });
  }
}
...
function interpretProjection(input: StageInterpretInputV1): ProgramInterpretationDraftV1 {
  try {
    ...
    const facts = boundFacts(input.program, input.facts, [payload.requestId]);
    const fact = returned(facts[0]!, "univ4.projection.state");
    const reserves = decodeReserveWords(fact.dataHex, "univ4.projection.state");
    const result = materializeUniv4({
      identity: payload.identityMemo.identity,
      read: { cutoff: payload.cutoff, instanceKey: payload.identityMemo.identity.instanceKey, reserveIn: reserves.reserveIn, reserveOut: reserves.reserveOut },
    });
    if (result.status === "chain-proven-rejected") return Object.freeze({ kind: "chainProvenRejected", factSet: input.factSet, decisionCode: result.reasonCode });
    if (result.state.stateHash !== payload.materialization.state.stateHash) throw new TypeError("univ4-projection-state-lineage-mismatch");
    if (result.state.identityFactsHash !== payload.identityMemo.identity.factsHash) throw new TypeError("univ4-projection-identity-lineage-mismatch");
    return Object.freeze({ kind: "verified", output: publication(payload, result.state) });
  } catch (error) {
    return Object.freeze({ kind: "invalidProgram", code: invalidCode(error, "univ4-projection-invalid") });
  }
}
...
function interpretRehydration(input: StageInterpretInputV1): ProgramInterpretationDraftV1 {
  try {
    ...
    const facts = boundFacts(input.program, input.facts, [payload.requestId]);
    const fact = returned(facts[0]!, "univ4.rehydration.reference");
    if (fact.dataHex !== ackData(payload.referenceHash)) throw new TypeError("univ4-rehydration-ack-mismatch");
    return Object.freeze({ kind: "verified", output: Object.freeze({
      kind: "univ4-rehydration-verified",
      binding: payload.binding,
      cutoff: payload.cutoff,
    ...
    return Object.freeze({ kind: "invalidProgram", code: invalidCode(error, "univ4-rehydration-invalid") });
  }
}
...
function definitionBase(
  ...
  return deepFreeze({
    ...
    outputSchemaRef,
    implementationClosureHash: hashDomain("aloha/univ4/runtime-implementation/v1", {
      stage,
      module: "families/univ4/src/runtime/definitions.ts",
    }),
    outputCodecHash: hashDomain("aloha/univ4/runtime-output-codec/v1", stage),
...
export const UNIV4_NOMINATION_RUNTIME = definitionBase(
  "nomination",
  nominationPayloadCodec,
  hashDomain("aloha/univ4/runtime-output-schema/v1", "nomination"),
  Object.freeze({ decodeExact: nominationOutput }),
  prepareNomination,
  interpretNomination,
);

export const UNIV4_IDENTITY_RUNTIME = definitionBase(
  "identity",
  identityPayloadCodec,
  hashDomain("aloha/univ4/runtime-output-schema/v1", "identity"),
  Object.freeze({ decodeExact: identityOutput }),
  prepareIdentity,
  interpretIdentity,
);

export const UNIV4_MATERIALIZATION_RUNTIME = definitionBase(
  "materialization",
  materializationPayloadCodec,
  hashDomain("aloha/univ4/runtime-output-schema/v1", "materialization"),
  Object.freeze({ decodeExact: materializationOutput }),
  prepareMaterialization,
  interpretMaterialization,
);

export const UNIV4_PROJECTION_RUNTIME = definitionBase(
  "projection",
  projectionPayloadCodec,
  hashDomain("aloha/univ4/runtime-output-schema/v1", "projection"),
  Object.freeze({ decodeExact: projectionOutput }),
  prepareProjection,
  interpretProjection,
);

export const UNIV4_REHYDRATION_RUNTIME = definitionBase(
  "rehydration",
  rehydrationPayloadCodec,
  hashDomain("aloha/univ4/runtime-output-schema/v1", "rehydration"),
  Object.freeze({ decodeExact: rehydrationOutput }),
  prepareRehydration,
  interpretRehydration,
);

export const UNIV4_NOMINATION_DEFINITION = UNIV4_NOMINATION_RUNTIME;
export const UNIV4_IDENTITY_DEFINITION = UNIV4_IDENTITY_RUNTIME;
export const UNIV4_MATERIALIZATION_DEFINITION = UNIV4_MATERIALIZATION_RUNTIME;
export const UNIV4_PROJECTION_DEFINITION = UNIV4_PROJECTION_RUNTIME;
export const UNIV4_REHYDRATION_DEFINITION = UNIV4_REHYDRATION_RUNTIME;

export const UNIV4_STAGE_DEFINITIONS: readonly FamilyStageDefinitionV1[] = Object.freeze([
...
export function requireUniv4StageDefinition(stage: FamilyRuntimeStageV1): FamilyStageDefinitionV1 {
  const definition = UNIV4_STAGE_DEFINITIONS.find(item => item.stage === stage);
  if (definition === undefined) throw new TypeError(`univ4 stage definition missing: ${stage}`);
  return definition;
}
