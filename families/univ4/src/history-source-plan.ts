...
export const UNIV4_HISTORY_SOURCE_PLAN_RUNTIME: FamilySourcePlanRuntimeV1 = Object.freeze({
  ...
  async execute(input: FamilySourcePlanExecutionInputV1, physical: FamilySourcePlanPhysicalPortV1, signal: AbortSignal) {
    ...
    if (input.plan.familyDefinitionHash !== UNIV4_FAMILY_DEFINITION_HASH || input.plan.completeness !== "contiguous-history" || input.plan.historyStartBlock !== "0") throw new TypeError("univ4 history source plan binding mismatch");
    const from = input.previousAppliedThrough === null ? "0" : decimal(BigInt(input.previousAppliedThrough) + 1n); if (BigInt(from) > BigInt(input.cutoff.number)) throw new TypeError("univ4 history cursor beyond cutoff");
    ...
    const opaqueResult: CanonicalJson = Object.freeze({ kind: "univ4-initialize-contiguous-history", version: 1, manager: MANAGER, topic: UNIV4_CONTRACT_EVIDENCE_TOPIC, from, through: input.cutoff.number, chunkBlocks: CHUNK_BLOCKS.toString(), entries: Object.freeze(entries) }); const resultPartitionRoot = hashDomain("aloha/univ4/history-source-partition/v1", opaqueResult); const withoutRoot = { kind: "source-plan-execution" as const, version: 1 as const, plan: input.plan, cutoff: input.cutoff, outcome: "complete" as const, from, through: input.cutoff.number, previousAppliedThrough: input.previousAppliedThrough, resultPartitionRoot, opaqueResult, sourceEvidenceRefs: refs, rawLocatorHashes, sourceEvidenceRoot: evidenceRoot };
