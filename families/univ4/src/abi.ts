...
/** Mainnet Uniswap v4 singleton bindings. These are infrastructure roots. */
export const UNIV4_POOL_MANAGER = "0x000000000004444c5dc75cB358380D2e3dE08A90" as const;
export const UNIV4_QUOTER = "0x52F0E24D1c21C8A0cB1e5a5dD6198556BD9E1203" as const;
export const UNIV4_STATE_VIEW = "0x7fFE42C4a5DEeA5b0feC41C94C136Cf115597227" as const;

export const UNIV4_GET_SLOT0_SELECTOR = "0xc815641c" as const;
export const UNIV4_GET_LIQUIDITY_SELECTOR = "0xfa6793d5" as const;
export const UNIV4_QUOTE_SELECTOR = "0xaa9d21cb" as const;
export const UNIV4_SWAP_SELECTOR = "0xf3cd914c" as const;

export type Univ4PoolKey = Readonly<{
...
export type Univ4InitializeLogV1 = Readonly<{
...
export function encodeQuoteCall(key: Univ4PoolKey, zeroForOne: boolean, amountIn: string): string {
  // quoteExactInputSingle((PoolKey,bool,uint128,bytes)) has one static tuple
  // followed by a dynamic hookData tail. The only supported family binding is
  // hook-free, so the tail is the canonical empty-bytes encoding.
  const body = [
  ...
  return `${UNIV4_QUOTE_SELECTOR}${body}`.toLowerCase();
}

export function encodeSwapCall(key: Univ4PoolKey, zeroForOne: boolean, amountIn: string): string {
  // PoolManager.swap(PoolKey,SwapParams,bytes), with hookData = 0x.
  const min = 4_295_128_740n;
  // The exact max value is checked in the family stage; keep the ABI helper
  // independent of a floating point literal by using the documented uint160
  // maximum.
  const sqrtPriceLimit = zeroForOne ? min : (1n << 160n) - 1n;
  const body = [
    addressWord(key.currency0, "poolKey.currency0"),
    addressWord(key.currency1, "poolKey.currency1"),
    uintWord(key.fee, "poolKey.fee"),
  ...
  return `${UNIV4_SWAP_SELECTOR}${body}`.toLowerCase();
}

export function decodeWords(value: string, count: number, path: string): readonly bigint[] {
  if (typeof value !== "string" || !HEX_BYTES.test(value) || (value.length - 2) % (WORD_BYTES * 2) !== 0) throw new TypeError(`${path} must be ABI words`);
  const available = (value.length - 2) / (WORD_BYTES * 2);
...
export function assertPoolKey(value: unknown, path = "poolKey"): Univ4PoolKey {
...
export function decodeUniv4InitializeLog(raw: Univ4InitializeLogV1, expectedTopic: Hash): Readonly<{
  readonly poolId: Hash;
  readonly poolKey: Univ4PoolKey;
}> {
  if (
    raw.address !== UNIV4_POOL_MANAGER.toLowerCase()
    || raw.topics.length !== 4
  ...
  ) throw new TypeError("univ4 Initialize log binding mismatch");
  const fee = abiUint(raw.data, 0, 24, "univ4.Initialize.fee");
  const tickSpacing = abiInt(raw.data, 1, 24, "univ4.Initialize.tickSpacing");
  const hooks = abiUint(raw.data, 2, 160, "univ4.Initialize.hooks");
  const sqrtPriceX96 = abiUint(raw.data, 3, 160, "univ4.Initialize.sqrtPriceX96");
  abiInt(raw.data, 4, 24, "univ4.Initialize.tick");
  if (sqrtPriceX96 === 0n) throw new TypeError("univ4 Initialize sqrtPriceX96 is zero");
  ...
  }, "univ4.Initialize.poolKey");
  const poolId = bytes32(raw.topics[1], "univ4.Initialize.poolId");
  if (poolIdForKey(poolKey) !== poolId) throw new TypeError("univ4 Initialize PoolKey reverse binding mismatch");
  return Object.freeze({ poolId, poolKey });
}

/** Keccak-256 (Ethereum variant), used only for PoolKey -> poolId binding. */
export function keccak256Hex(input: Uint8Array): Hash {
...
export function poolIdForKey(key: Univ4PoolKey): Hash {
