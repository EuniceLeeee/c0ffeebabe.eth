...
import { encodeSwapInCall } from "../src/abi.ts";
import { FLUID_DEX_ACTION_MUTATION_CORPUS_ROOT, FLUID_DEX_ACTION_MUTATION_EXECUTION_IDS, FLUID_DEX_ACTION_PORT, buildFluidDexSearchAction } from "../src/action.ts";
...
const action = buildFluidDexSearchAction({ rawAction, quote, route, token0: route.inputAsset, token1: route.outputAsset, swap0to1: true, recipient, stateFactsRoot, inputs: [{ assetRef: erc20AssetRefV1("1", route.inputAsset), amount: "100" }], outputs: [{ assetRef: erc20AssetRefV1("1", route.outputAsset), amount: "97" }], exactEvaluationHash, obligationRoot });

test("Fluid action owner exactly verifies its semantic payload and obligation proof", () => {
  assert.equal(FLUID_DEX_ACTION_PORT.decode(action).actionHash, action.actionHash);
  ...
  const flippedProofRoot = hashDomain("aloha/fluid-dex/action-obligation-postcondition/v1", { rawActionHash: flippedRawAction.actionHash, routeBindingHash: route.routeBindingHash, token0: route.inputAsset, token1: route.outputAsset, exactEvaluationHash: flippedEvaluationHash, obligationRoot: flippedObligationRoot, inputs: action.inputs, outputs: action.outputs, stateFactsRoot, swap0to1: false, recipient });
  const flippedPayload = { ...action, rawAction: flippedRawAction, swap0to1: false, exactEvaluationHash: flippedEvaluationHash, obligationRoot: flippedObligationRoot, obligationProofRoot: flippedProofRoot };
  const { actionHash: ignoredFlipped, ...flippedBody } = flippedPayload;
  void ignoredFlipped;
  const mutationCorpus = [
    ...
    { id: "direction-reroot", value: { ...flippedBody, actionHash: hashDomain("aloha/fluid-dex/search-action/v1", flippedBody) } },
  ] as const;
  assert.deepEqual(mutationCorpus.map(mutation => mutation.id), FLUID_DEX_ACTION_MUTATION_EXECUTION_IDS);
  assert.equal(FLUID_DEX_ACTION_MUTATION_CORPUS_ROOT, hashDomain("aloha/fluid-dex/action-verifier-mutations/v1", { executionIds: FLUID_DEX_ACTION_MUTATION_EXECUTION_IDS }));
  ...
  const cutoffProof = hashDomain("aloha/fluid-dex/action-obligation-postcondition/v1", { rawActionHash: cutoffRaw.actionHash, routeBindingHash: route.routeBindingHash, token0: route.inputAsset, token1: route.outputAsset, exactEvaluationHash, obligationRoot, inputs: action.inputs, outputs: action.outputs, stateFactsRoot, swap0to1: true, recipient });
  ...
  void ignoredCutoff;
  reject({ ...cutoffBody, actionHash: hashDomain("aloha/fluid-dex/search-action/v1", cutoffBody) });
});
