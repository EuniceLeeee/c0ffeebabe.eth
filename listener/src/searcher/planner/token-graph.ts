...
//
// The legacy eth_call graph builder (buildTokenGraph /
// buildTokenGraphWithResults) was physically removed: strict startup
// rebuild + frozen ready topology are the only graph authority, and
// compile errors expose gaps rejects reconnecting any legacy runtime
// builder. Consumers that still need per-pool edges must derive them
// from the strict catalog lifecycle, never from a parallel chain query.
