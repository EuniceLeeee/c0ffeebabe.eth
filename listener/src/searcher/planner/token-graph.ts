...
const EXTERNAL_AND_LEGACY_POOL_REGISTRY: PoolEntry[] = [
  // Standard ERC4626 vault seeds are fully migrated to adapter-owned discovery
  // (event/DEX enumeration -> identity attest -> route probe -> evidence cache).
  // The former hardcoded vault list is retained ONLY as the expected-recall
  // answer key in the no-seed replay acceptance (see erc4626 discovery tests);
  // it is never a discovery candidate source. buildEdges is verified-routes
  // gated, so a dynamically rejected leg cannot be regrown from a seed.
  //
