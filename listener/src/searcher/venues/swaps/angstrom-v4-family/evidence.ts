...
import { assertSameSource, canonicalPoolKey, sameAddress } from "./codec.js";
 ...
 * empty block); the first current-source EOA-verified attestation becomes
 * execution evidence bound to the observed instance. This projection does
 * not prove current validator membership: the pending authority path and
 * the hook-aware quoter/execution retain their authorization checks.
 * The central descriptor builder executes this
...
export function angstromRuntimeEvidenceFromObservation(input: {
  ...
  if (
    ...
    !ethers.isHexString(input.observation.transactionHash, 32) ||
    input.observation.target.toLowerCase() !== ANGSTROM_MAINNET_ADAPTER.toLowerCase()
  ) {
    return Object.freeze([]);
  }
  const swapIface = new ethers.Interface(ANGSTROM_ADAPTER_SWAP_ABI);
  try {
    assertSameSource(input.observation.source, input.source);
    ...
    const key = canonicalPoolKey({
      currency0: String(decoded.key.currency0),
      currency1: String(decoded.key.currency1),
      fee: Number(decoded.key.fee),
      tickSpacing: Number(decoded.key.tickSpacing),
      hooks: String(decoded.key.hooks),
    });
    if (!sameAddress(key.hooks, ANGSTROM_MAINNET_HOOK)) return Object.freeze([]);
    const observedInstanceKey = instanceKey(
      `${ADDR.UNISWAP_V4_POOL_MANAGER.toLowerCase()}\u001f${v4PoolId(key)}`,
    );
    ...
    for (const item of hookData) {
      ...
      if (
        ...
        blockNumber !== BigInt(input.source.number) ||
