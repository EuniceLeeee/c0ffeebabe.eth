...
import { nominateUniv4 } from "../univ4-family/nomination.js";
import { reverseBindUniv4 } from "../univ4-family/reverse-binding.js";

export const UNIV4_POOL_SURFACE_PATTERN_ID = "univ4-pool-surface";

/**
 * Identical nomination surface to the standard univ4 Family (the manager
 * emits the same Initialize/Swap events regardless of hook); identity is
 * what separates the fee-hook pools.
 */
export const univ4FeeHookDiscovery = {
  ...
  decodeCandidate({ observation, matchedPatternId }) {
    try {
      return decodeFeeHookCandidate(observation, matchedPatternId);
    } catch {
      return null;
    }
  },
...
} satisfies DiscoverySemantics<FeeHookCandidate>;

function decodeFeeHookCandidate(
