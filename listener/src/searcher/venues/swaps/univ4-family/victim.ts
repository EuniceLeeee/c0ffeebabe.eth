...
export const univ4VictimReplay = {
  ...
  buildOverlay({ descriptor, route, impact }) {
    assertImpactBinding(descriptor, route, impact);
    // PoolManager replay is represented by its exact landed post-state; there
    // is no family-owned transaction overlay in the legacy production closure.
    return null;
  },
} satisfies VictimReplaySpec<UniV4Descriptor, UniV4Route>;
...
function assertImpactBinding(
  ...
  if (
    ...
    throw new Error("univ4 victim impact does not match the bound route");
  }
}
...
function exactPostState(
  ...
  if (
    ...
    throw new Error("univ4 victim exact post-state poolId mismatch");
  }
  return Object.freeze({
    kind: "v4",
...
function canonicalRecord(
  ...
  if (value === null || Array.isArray(value) || typeof value !== "object") {
    throw new Error("univ4 victim exact post-state must be a canonical record");
  }
...
function canonicalBigint(
  ...
  if (typeof value !== "bigint") {
    throw new Error(`univ4 victim ${label} must be bigint`);
  }
  return value;
}
...
function canonicalInteger(
  ...
  if (typeof value !== "number" || !Number.isSafeInteger(value)) {
    throw new Error(`univ4 victim ${label} must be a safe integer`);
  }
  return value;
}
