    ...
    const candidates = await mapLimit(
      ...
      async (item): Promise<ProtocolCandidate | null> => {
        ...
        for (const log of item.logs.slice(-3).reverse()) {
          ...
          // caller retries the same block range. A local node's explicitly
          // pruned historical state is permanent, however: fail this candidate
          // closed and continue so one old event cannot poison every newer
          // candidate in the discovery window.
          let verified: Erc4626PayoutEvidence | null;
          try {
            verified = await verifyWithdrawPayout(ctx, item.target, asset, log);
          } catch (error) {
            if (isPermanentlyPrunedHistoricalState(error)) continue;
            throw error;
          }
          if (verified) {
...
function isPermanentlyPrunedHistoricalState(error: unknown): boolean {
  const message = error instanceof Error ? error.message : String(error);
  return /state at block\s*#?\d+\s+is pruned/i.test(message) ||
