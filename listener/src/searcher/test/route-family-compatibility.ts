...
function testOneToOneProjection(): void {
  ...
  assert(adapters.length === 16, `expected 16 production adapters, got ${adapters.length}`);
...
function testDynamicAdmission(): void {
  ...
  assert(dynamic.length === 2, `expected two dynamic families, got ${dynamic.length}`);
  ...
  assert(
    dynamic[1]?.executionFamilyId === "protocol:eigenpie" &&
      dynamic[1]?.dynamicAdmission?.candidateSources.join(",") === "observed-interaction",
...
function testReceiptDepositFrameworkIsNotARegisteredFamily(): void {
  const erc4626 = PRODUCTION_ROUTE_ADAPTERS.protocols.find(
    (adapter) => adapter.id === "protocol:erc4626",
  );
  const eigenpie = PRODUCTION_ROUTE_ADAPTERS.protocols.find(
    (adapter) => adapter.id === "protocol:eigenpie",
  );
  assert(erc4626 !== undefined && eigenpie !== undefined, "receipt-deposit users missing");
  assert(
    !PRODUCTION_ROUTE_ADAPTERS.protocols.some(
...
function testFieldAndOrderSnapshot(): void {
  const expected = [
    ...
    "protocol:erc4626|protocol-conversion|erc4626|erc4626-deposit,erc4626-redeem,erc4626-redeem-silo|erc4626-deposit,erc4626-redeem,erc4626-redeem-silo,erc20-approve|0|true|dex-token-domain,observed-interaction|true",
    ...
    "compat:fluid-credit|compat|fluid-vault|fluid-vault|fluid-vault,erc20-approve|0|false|-|-",
  ] as const;
