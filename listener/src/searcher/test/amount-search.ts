...
async function testBlockScanSearchBudgetTelemetry(): Promise<void> {
  ...
  assert(timing.amountPoints === 8, `solver amount points ${timing.amountPoints}`);
  assert(timing.gssPoints === 4, `solver GSS points ${timing.gssPoints}`);
  assert(timing.hopExactCalls === 16, `solver must reuse searched handles for all 3 finalists: ${timing.hopExactCalls}`);
  ...
  console.log("[amtsearch] block-scan 4+4 search budget telemetry: PASS");
}
