...
async function main(): Promise<void> {
  ...
  const cachedClient = new MockClient();
  const cachedBackend = backend(cachedClient, [HASH_B]);
  const cachedState = cachedBackend as unknown as {
    victimOverlayReadBlock: number | null;
    victimOverlayReadCache: Map<string, string>;
  };
  cachedState.victimOverlayReadBlock = 100;
  cachedState.victimOverlayReadCache.set("100:stale", "0x01");
  await expectReject(
    cachedBackend.prepareVictimState(input()),
    "reorged before prepare",
  );
  assert(
    cachedState.victimOverlayReadBlock === null,
    `reorg cache block ${cachedState.victimOverlayReadBlock}`,
  );
  assert(
    cachedState.victimOverlayReadCache.size === 0,
    `reorg cache entries ${cachedState.victimOverlayReadCache.size}`,
  );

  console.log("revm-canonical-anchor PASS (4/4)");
}
