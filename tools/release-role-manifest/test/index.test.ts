...
export const ADAPTER_ONE_ALT = ADAPTER;\n`);
  writeFileSync(join(root, "acceptance", "gate-core", "src", "provider-one.mjs"), "export const PROVIDER_ONE = Object.freeze({ predicateId: 'fixture.one', providerContractVersion: '1.0.0', providerContractDigest: '" + hash("8") + "', async provide() { return Object.freeze({}); } });\n");
  ...
  writeFileSync(join(root, "tools", "runtime-release-packager", "src", "internal", "qualified-release-runner-owner.ts"), "export async function runQualifiedReleaseAcceptanceV1() { return Object.freeze({}); }\nexport const runQualifiedReleaseAcceptanceV1Alt = runQualifiedReleaseAcceptanceV1;\n");
...
export const RELEASE_ROLE_COMPOSITION = Object.freeze({
  ...
  qualifiedRunner: { modulePath: 'tools/runtime-release-packager/src/internal/qualified-release-runner-owner.ts', exportName: 'runQualifiedReleaseAcceptanceV1' },
  ...
  predicateAdapters: [{
    ...
    materialProviderModulePath: 'acceptance/gate-core/src/provider-one.mjs',
    materialProviderExportName: 'PROVIDER_ONE',
    materialProviderContractDigest: '${hash("8")}'
  }]
});
...
test("production generation owns the exact three generated outputs and one fixed authority placeholder", async () => {
  ...
  const productionInputs = result.ledger.inputFiles.map((entry) => entry.path);
  assert.ok(productionInputs.includes("tools/runtime-release-packager/src/internal/qualified-release-runner-owner.ts"));
  assert.ok(productionInputs.includes("tools/runtime-release-packager/src/internal/runtime-binding-verifier.ts"));
  assert.equal(productionInputs.includes("tools/runtime-release-packager/src/deployment-package.ts"), false);
  assert.equal(productionInputs.some((path) => path.startsWith("apps/searcher-runtime/")), false);
  ...
  assert.match(composition.text, /materialProviderImplementationExportDigest:/);
  assert.match(composition.text, /materialProvider: materialProvider0/);
...
test("generator binds real module exports, compiler entrypoint IDs, and content ledger", async () => {
  ...
  try {
    ...
    assert.equal(result.manifest.commonEnvelopeRoleContractVersion, "1.0.0");
    assert.equal(result.manifest.predicateAdapters[0]?.commonEnvelopeRoleContractVersion, "1.0.0");
    ...
    assert.equal(result.manifest.qualifiedRunner.entrypointId, "compiler-root:tsconfig.json:tools/runtime-release-packager/src/internal/qualified-release-runner-owner.ts");
    assert.equal(result.manifest.qualifiedRunner.implementationExportDigest, computeImplementationExportDigest(root, {
      modulePath: "tools/runtime-release-packager/src/internal/qualified-release-runner-owner.ts",
      exportName: "runQualifiedReleaseAcceptanceV1",
    }));
    const baselineRunner = result.manifest.qualifiedRunner;
    ...
    assert.equal(result.manifest.predicateAdapters[0]?.materialProviderEntrypointId, "compiler-root:acceptance/gate-core/tsconfig.json:acceptance/gate-core/src/provider-one.mjs");
    ...
    assert.equal(baselinePredicate.compositionLeafDigest, computePredicateCompositionLeafDigest({
      ...
      materialProviderModulePath: baselinePredicate.materialProviderModulePath,
      materialProviderExportName: baselinePredicate.materialProviderExportName,
      materialProviderContractDigest: baselinePredicate.materialProviderContractDigest,
      materialProviderImplementationExportDigest: baselinePredicate.materialProviderImplementationExportDigest,
    }));
    ...
    writeFileSync(compositionPath, originalComposition.replace("exportName: 'runQualifiedReleaseAcceptanceV1'", "exportName: 'runQualifiedReleaseAcceptanceV1Alt'"));
    const alternateRunner = await generateReleaseRoleManifest(config);
    assert.notEqual(alternateRunner.manifest.qualifiedRunner.implementationExportDigest, baselineRunner.implementationExportDigest);
    assert.notEqual(alternateRunner.manifest.rootDigest, result.manifest.rootDigest);
    writeFileSync(compositionPath, originalComposition);

    const runnerPath = join(root, "tools", "runtime-release-packager", "src", "internal", "qualified-release-runner-owner.ts");
    writeFileSync(runnerPath, `${readFileSync(runnerPath, "utf8")}\n// runner mutation\n`);
    assert.ok(verifyReleaseRoleManifestLedger(root, result.ledger).some((entry) => entry.startsWith("input-content:tools/runtime-release-packager/src/internal/qualified-release-runner-owner.ts")));
...
test("exact check rejects stale generated bytes without writing", async () => {
  ...
  try {
    ...
    for (const output of result.generatedOutputs) {
      const jointlyForgedOutput = `${output.text}\n// jointly forged output and ledger\n`;
      writeFileSync(join(root, output.path), jointlyForgedOutput);
      const forgedRecord = forgedOutputs.find((entry) => entry.path === output.path);
    ...
    for (const output of result.generatedOutputs) {
      assert.ok(jointlyForgedErrors.includes(`generated-content:${output.path}`));
    }
...
test("generator rejects duplicate or non-static predicate bindings without executing candidate modules", async () => {
  ...
  try {
    ...
    writeFileSync(runnerPath, "export interface runQualifiedReleaseAcceptanceV1 { readonly typeOnly: true }\n");
    await assert.rejects(() => generateReleaseRoleManifest(config), /missing direct static export/);
    writeFileSync(runnerPath, "export declare function runQualifiedReleaseAcceptanceV1(): Promise<unknown>;\n");
    await assert.rejects(() => generateReleaseRoleManifest(config), /missing direct static export/);
    writeFileSync(runnerPath, runnerSource);
    writeFileSync(compositionPath, source.replace("  qualifiedRunner: {", "  qualifiedRunner: { modulePath: 'tools/runtime-release-packager/src/internal/qualified-release-runner-owner.ts', exportName: 'runQualifiedReleaseAcceptanceV1' },\n  qualifiedRunner: {"));
    await assert.rejects(() => generateReleaseRoleManifest(config), /duplicate property qualifiedRunner/);
    writeFileSync(compositionPath, source.replace("commonEnvelopeRoleContractVersion: '1.0.0'", "commonEnvelopeRoleContractVersion: '2.0.0'"));
    await assert.rejects(() => generateReleaseRoleManifest(config), /unsupported common envelope role contract version/);
    ...
    writeFileSync(compositionPath, source);
    const oraclePath = join(root, "acceptance", "oracle", "src", "one.mjs");
    ...
    writeFileSync(oraclePath, `export const side = import('./late.mjs'); export const ORACLE_PROGRAM_DESCRIPTOR_DIGEST = '${hash("7")}'; export const ORACLE_VERSION = 'oracle-v1'; export function oracleOne() { return 'oracle'; }\n`);
    await assert.rejects(() => generateReleaseRoleManifest(config), /dynamic import/);
    writeFileSync(oraclePath, oracleSource);
    writeFileSync(join(root, "tools", "runtime-release-packager", "src", "internal", "runner-late.ts"), "export const late = 1;\n");
    writeFileSync(runnerPath, "void import('./runner-late.ts'); export async function runQualifiedReleaseAcceptanceV1() { return Object.freeze({}); }\n");
    await assert.rejects(() => generateReleaseRoleManifest(config), /dynamic import/);
    writeFileSync(runnerPath, runnerSource);
...
test("generator compiler closure rejects dynamic loaders and records transitive helpers", async () => {
  ...
  try {
    ...
    writeGeneratedReleaseRoleManifest(result, config);
    assert.deepEqual(await checkGeneratedReleaseRoleManifest(config), []);
    writeFileSync(generatorIndexPath, "import { helper } from './helper.ts'; const registry = { require(_key: string) { return helper; } }; export const generator = registry.require(String(Date.now()));\n");
    await assert.doesNotReject(() => generateReleaseRoleManifest(config));
    writeFileSync(generatorIndexPath, "import { helper } from './helper.ts'; export const generator = helper;\n");
...
test("an unrelated predicate changes only its leaf and aggregate roots", async () => {
  ...
  try {
    ...
    writeFileSync(join(root, "acceptance", "gate-core", "src", "provider-two.mjs"), "export const PROVIDER_TWO = Object.freeze({ predicateId: 'fixture.two', providerContractVersion: '1.0.0', providerContractDigest: '" + hash("a") + "', async provide() { return Object.freeze({}); } });\n");
    ...
    materialProviderModulePath: 'acceptance/gate-core/src/provider-two.mjs',
    materialProviderExportName: 'PROVIDER_TWO',
    materialProviderContractDigest: '${hash("a")}'
  }]\n});`);
