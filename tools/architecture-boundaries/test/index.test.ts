...
test("qualified release runner owns the only CommonEnvelope issuer and generated runtime seam", () => {
  ...
  const inspect = (
    source: string,
    edges: readonly GraphEdge[] = [issuerEdge, runtimeEdge],
  ...
  try {
    ...
    assert.ok(inspect(
      ...
      [issuerEdge, { ...runtimeEdge, specifier: alternateRuntimeSpecifier }],
...
test("predicate material issuer and reader have separate exact owner edges", () => {
  ...
  const edges: GraphEdge[] = [
    ...
    { from: sharedPath, to: statePath, specifier: stateFromShared },
