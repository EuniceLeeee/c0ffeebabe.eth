...
- **F5 live 验收运行（f588b5dd，3h 窗口，backend=revm，subset）**：
  实测完整 ret13（12,015 池）生产 startup attestation 28,408 行耗时
  超 3h（universe 12k + blockscan 12k 两轮全量 + retained ~4k，冷池
  address-surface fallback 每池 ~4-19s），90min 与 3h 单窗口均无法到达
  periodic pass（EVENTS=0）——完整 universe 的 F5 live 验收在合理窗口
  不可行。改为 fresh-only 验收 subset（1,273 池：univ2 400/univ3 400/
  univ4 400/dodo 50/fluid-dex 21/angstrom 2，覆盖全部 fresh 族）作
  universe 输入 + `SEARCHER_DISCOVERY_BACKFILL_ENABLED=0` +
  `SEARCHER_DISCOVERY_HOT_DEX_ENABLED=0`（checkpoint 既有冻结做法），
  预期 startup ~15min 后 ~2.5h periodic pass 产出 events/routes；
