...
- complete-snapshot verifier-gated 一次性消费 + address-surface 正向路径
  （wsteth 真实 lifecycle → closure receipt → prepare/publish）；
- factory-log incumbent bootstrap（univ2-standard 风格族合同级关闭，
  candidateKey === inventoryKey、hash v3、staged-set mismatch
  fail-closed）；
- durable checkpoint v2 incumbent inventory +
  `CheckpointDiscoveryInventoryEnumerator` 接线（main env gate 下替代
  throw 占位，枚举失败保持 append-only）；
...
**2026-08-12 mixed-mode subset closure checkpoint（commit 见下，
§1 验收 4 合同级落地；真实生产 catalog 的 13 个 bootstrap-eligible 族可
关闭，其余 7 族保持 append-only）：**

- snapshot inventory closure 改为 subset 语义：一个 receipt 精确覆盖
  candidate 中的 bootstrap-eligible Family 子集（非空），非 eligible
  族不能入 receipt；权威枚举对 receipt 覆盖子集生效，receipt 外的
  Family 按构造保持 append-only；
- 真实生产 catalog 合同：22 族中 20 个 discovery 族，13 个
  bootstrap-eligible（9 address-surface + 4 factory-log），7 个
  observed-call/landed-log-only 族仍无 bootstrap 语义；全 eligible
  13 行 closure 签发成功，univ2-standard 单族 subset closure 签发并
  一次性消费成功，混入非 eligible 族 / 空 candidate 均 fail closed；
- composition 混合模式端到端：wsteth（complete-snapshot + closure
  receipt）与 astra-multitoken（append-only-delta 空 stage，
  append-only-nomination anchor）同批 prepare/commit 成功；envelope
  中 wsteth 为 complete-snapshot、astra 为 append-only-delta，
  snapshot.status 如实为 shadow-partial（astra 无 bootstrap 语义前
  不得声称 complete）；
- 证据：shadow suite（11 合同）全过 + regression sweep 11 组全过
  （含 parity verifier 与 node SSM evidence verifier）+ 完整 build；
  本地 sealed-capture/shadow receipt 范围，不代表 production cutover。
...
- 用户确认：astra/eigenpie/erc4626-silo/ethertoken/hgusdc/
  curve-underlying/dodo-v2 这 7 个仅以 observed-call/landed-log 为
  discovery 证据的 Family **不开发 complete-snapshot bootstrap
  语义**，按 MEV 范围有意保持 append-only / positive-only；
- 理由：没交易过的池子不存在套利机会，不列入是范围而非 coverage
  bug；graph 是已成功解析的活跃池的增量缓存，唯一需 fail-closed 的
  失败类是协议解析失败（已有 fail-closed 解码 + semantic-mismatch
  gate）；活动证据**不得**行使 complete-snapshot 删除权（避免误删
  暂时安静、之后活跃的池子）；
- 影响：canonical"未关闭"列表与 Phase D 表移除"7 族 bootstrap 缺口"
...
**2026-08-12 erc4626-silo-redeem bilateral parity phase checkpoint（实现
commits impl `643e1b86`、baseline `c5c7af4c`，本地机器证据）：**

- fixture runtime 首次为 effect-delta-simulation 接上 verified-actor 与
  executor 中央 caller authority（此前 fixture 一律空 authority，identity
  active 步骤报 `authority-failure`）；`identity-active-redeem` 与
  `exact-active-redeem` 的 fixture 结果带 token-delta/total-supply-delta
  效果集（1:1 redeem，actor 由 callerRef 解析）；
- impl `captureErc4626SiloRedeemFixtureCase`（observed multi-token redeem、
  custom identity base→preview→active、dependent preview chain
  pricing、active-redeem exact simulation）全部 10 stage `exercised`；
  normalizer 六类 stage（`address-subinstance` venue、composite
  `vault:payout` instance key、`redeem` 行为、silo-multitoken 静态投影）
  落地；
- baseline exporter `captureErc4626SiloRedeemBaselineCase`（1:1 preview
  chain、active redeem effects、canonicalHash 镜像）；
- 本地跨分支 parity（block `25729060`，十二族 manifest）：十二族全部
  **pass**、commonGraph 五 stage delta 全空；
  `searcher:architecture-migration-capture` 合同测试与完整
  `npm run build` 全部通过；仍是 sealed-capture/shadow receipt，不是
  production cutover 或 live evidence。

剩余 10 族：`credit:fluid`、`curve-underlying`、`custom-swap:angstrom-v4`、
...
**2026-08-12 self-burn-native bilateral parity phase checkpoint（实现
commits impl `8fbe205d`、baseline `9a511ec9`，本地机器证据）：**

- fixture caller authority 同时绑定 probe 与 pricing 两个 verified actor
  evidence id（`self-burn-native-probe-actor`、
  `self-burn-native-pricing-actor`）；identity/current/exact 的
  transfer-self effect-delta 结果带 token/native/total-supply delta
  （1:1 burn，pricing 取 1e16 首探针）；
- impl `captureSelfBurnNativeFixtureCase`（observed transfer-self、
  custom identity base→active、probe-based variable-native-out pricing、
  两节点 redeem+WETH deposit execution fragment）全部 10 stage
  `exercised`；normalizer 六类 stage（transfer-self 静态投影、native
  anchor、`redeem` 行为）落地；
- baseline exporter `captureSelfBurnNativeBaselineCase`（probe pricing
  1e16、exact 1:1、双节点 canonicalHash）；
- 本地跨分支 parity（block `25729060`，十五族 manifest）：十五族全部
  **pass**、commonGraph 五 stage delta 全空；
  `searcher:architecture-migration-capture` 合同测试与完整
  `npm run build` 全部通过；仍是 sealed-capture/shadow receipt，不是
  production cutover 或 live evidence。

剩余 7 族：`credit:fluid`、`curve-underlying`、`custom-swap:angstrom-v4`、
...
**2026-08-12 fluid-dex bilateral parity phase checkpoint（实现 commits
impl `c9bc313a`、baseline `830c6f5a`，本地机器证据）：**

- 修复 fluid-dex pricing `dependencies` 重复项（quoteBinding.target 与
  pool 重复）；
- impl `captureFluidDexFixtureCase`（observed swapIn、factory-child
  constants→reverse-binding→双向 declared-revert quote、两条 direction
  全部 10 stage `exercised`）；normalizer 六类 stage（`address-pool`
  venue、`external-swap` quoted mid、factory/quote binding 投影）落地；
- baseline exporter `captureFluidDexBaselineCase`（constants/dexId/
  reverse binding、declared revert 1:1 quote、swap0to1 执行参数）；
- 本地跨分支 parity（block `25729060`，十九族 manifest）：十九族全部
  **pass**、commonGraph 五 stage delta 全空；
  `searcher:architecture-migration-capture` 合同测试与完整
  `npm run build` 全部通过；仍是 sealed-capture/shadow receipt，不是
  production cutover 或 live evidence。
...
**2026-08-12 S1 regression sweep harness checkpoint（commit 见下，
ops/CI gate，非部署）：**

- 新增 `scripts/s1-regression-sweep.sh`：一条命令跑完整 listener
  build、architecture-migration-capture、shadow suite、parity runner、
  default-authority/systemic-live/cutover-readiness/startup-manifest/
  consumer-diagnostic 全部合同，再跑 committed 22-family parity
  verifier；输出逐测试日志与
  `s1-regression-receipt.json`（implCommit/passedTests/时间窗）；
- 本轮实测：10 组全过（含 parity verifier，commit
...
2. 先记录本轮人工批准的 40 位 commit 为 `<APPROVED_SHA>`；fetch 后必须用
   ...
   startup log `runtime_commit`（systemd；禁止 nohup；dry-run 保持）。
