...
| A | 同上 | plugin-owned `execution.runtimeProjection` → generated catalog → strict live backend | **2026-08-13 strict-side checkpoint（未完成删除）：** 原中央 20-Family execution projection import/map 已删除；20 个插件本地显式声明 spender/prewarm，框架测试改为 synthetic catalog。build/定向合同/shadow suite 通过；旧 node enumerator receipt 因 catalogHash 改变诚实 fail closed，待固定 commit 节点 dry-run 刷新。legacy `quoteByAdapter`/`overlayApproveSpender` fallback 仍在，因此本行不构成 Pair A 或 F8 完成。 |
