...
| A | 同上 | plugin-owned `execution.runtimeProjection` → generated catalog → strict live backend | **2026-08-14 strict 侧完成（commit 686a5689）**：中央 20-Family execution projection import/map 已删除（08-13）；`overlayApproveSpender` 改 strict-only（composition 默认后 strict execution 投影是唯一 overlay 权威，无投影 fail-closed 返回 null，不再查询 legacy routes()）；prewarm/encodeHopQuoteCalls 已 strict-first。legacy `quoteByAdapter` exact 分支保留为 solver backstop，删除依赖 Pair E strict quote 100% 覆盖（见 Pair E 待收口项）。节点 dry-run 待 fixed commit 刷新。 |
...
| C | `landedPoolDiscovery` / `landed-event-registry` / `auto-close-router-gap` 消费 | strict discovery checkpoint + enumerator + observed-complete 事件面 | **2026-08-14 strict 侧完成（commit 8efcf4a0）**：durable discovery continuity composition 默认启用（checkpoint 路径 env 可覆盖，默认 `searcher/state/discovery-continuity-checkpoint.json`）；启动 loadForRestart（empty/degraded/trusted）→ enumerator 恢复 durable incumbent（fail-closed）→ writer 在每次 live publication 后 CAS 写入；strict 观察链（address-surface + event observations → lifecycle → catalogRoot CAS）由 onPublicationApplied 触发。节点 dry-run：`discovery continuity composition empty` 正常启动、无 fatal。删除前置：landedPoolDiscovery/landed-event-registry 为通用 registry（plugin 声明派生，非逐族表），blockscan 拓扑等消费合法保留；确认无 shadow-only 权威残留 |
| D | `productionPoolUniverseSourceFingerprints`（universe deploy trust） | strict catalog/checkpoint 派生指纹（identity/lineage 部分先由 strict 覆盖） | **2026-08-14 strict 侧完成（commit d70b1c77）**：新增 `productionPoolUniverseSourceFingerprintsStrict()`——strict-catalog-universe:v1 指纹从生成式 catalog 派生（catalogHash + 每族 definitionBoundaryHash + ownedActionAdapterIds + identity lineageIds + discoverySources，零逐族 switch）；main.ts 启动 / build-active-pool-universe / pool-universe-deploy-trust 全部改用 strict 版。合同测试（非空/确定性/格式/稳定性）并入 shadow suite（55/55）。节点 dry-run：已部署 universe 指纹 mismatch → fail-closed `registry_source_mismatch`，扫描从 max(universe.toBlock, cursor) 恢复，无 fatal。删除前置：legacy identity-policies 指纹仅保留为 strict 的过渡内部组件 |
| E | blockscan loop 的 legacy pricing 消费（14 legacy pricing Family） | strict pricing views + `strict-solver-consumer` 全量解析 | **2026-08-14 收口完成（commit 6589823e）**：`createStrictQuoteSource` 增加 fallback 策略，main.ts 在 durable composition（Pair C 默认）下接 `fail-closed`——solver 对 committed views 未覆盖的 route 抛错而非回退 legacy，solver 不再经非 strict 路径报价。合同测试扩展 fail-closed 用例（unknown/no-views reject、legacy 零调用）；legacy fallback 仅保留为 transitional 默认。节点 dry-run：`no committed pricing` 计数 0、无 fatal、240s 正常。shadow suite 55/55、sweep 12/12 |
| F | family facade / 手工 revision / legacy schema/cache bridge / 旧 flag | strict manifest + capability hash + CAS publication | **2026-08-14 strict 侧完成（commit b9ab331d）**：blockscan state schema/cache revision 从 strict catalog definitionBoundaryHash 派生（registerBlockScanStateFamily 接受 strictDefinitionBoundaryHash；AdapterFamilyRegistry 接 strict catalog 投影；PRODUCTION_ADAPTER_FAMILIES 接线）。运行时验证：19 个 blockscan family 全部使用 strict 64-hex schema revision，声明变更自动失效 blockscan state cache（无需手工 bump）。blockscan-runtime-startup-warm 42/42，shadow suite 55/55，sweep 12/12。删除前置：legacy family facade（assembleSchema/compileStaticSchema）与手工 adapterSchemaRevision 的 blockscan 消费待 Pair E 收口后删除 |
