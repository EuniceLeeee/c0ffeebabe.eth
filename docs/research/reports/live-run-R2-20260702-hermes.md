...
| **Coverage frontier (non-standard / native-ETH / v3-fork pool population)** — same `gap_class` in ≥3 independent samples in one window (R1 33 + R2 17 `no_candidate` on non-overlapping pool sets; Slice-2 window: original sample + coffeebabe `0x2e8b0b…` + ae2Fc483 sample 5/8 pools OUT) | — | epic | **decision: epic** (rule 13 trigger fired). Out of the 30-min loop. Slices: W1 governance trigger (done) → W2 `coverage_kpi` instrument → W3 learn→close auto-enqueue (probe/merge closable OUT pools) → adapter work joins the v4 epic where shapes overlap. Plan: `docs/research/hermes-coverage-process-plan.md`. Per-pool pins for this class inside the loop now forbidden. |
