...
import type { PreReleaseAuthorizationClaimCapabilityV1 } from "../../../tools/runtime-release-packager/src/pre-release-staging-contract.ts";
import {
...
import { readFixedPreReleaseAuthorizationClaimV1 } from "../../../tools/runtime-release-packager/src/internal/pre-release-authorization-ledger.ts";
import {
...
const PRE_RELEASE_RESTART_NAMESPACE_PREFIX = "pre-release-restart-terminal-v1";
const PRE_RELEASE_RESTART_DOMAIN = "aloha/pre-release-restart-terminal/v1";
...
function decodeStoredEvent(value: unknown, expectedRelease: ReleaseIdentityV1): RuntimeAcceptanceProcessEventV1 {
  ...
  if (record.kind === "aloha.runtime-process-ready") {
    const embeddedAnchor = decodeProcessAnchor(record.processAnchor as object);
    if (hashProcessAnchor(embeddedAnchor) !== record.processAnchorHash) {
      throw new TypeError("runtime process-ready embedded process anchor hash mismatch");
    }
  }
    ...
    if (input.phaseManifest.kind === "production") {
      ...
      phaseManifest = Object.freeze({
        ...
        processCommandSha256: sha256Hex(Buffer.from([
          "/usr/bin/node",
          PRE_RELEASE_STAGING_LAYOUT_V1.launcherPath,
          "",
        ].join("\0"))),
      });
    } else {
...
function preReleaseRestartNamespace(releaseProvenanceHash: Hash, stagingManifestRoot: Hash): string {
  return `${PRE_RELEASE_RESTART_NAMESPACE_PREFIX}:${releaseProvenanceHash.slice(2)}:${stagingManifestRoot.slice(2)}`;
}

function physicalStore(path: string, label: string): Readonly<{ readonly path: string; readonly device: string; readonly inode: string }> {
  if (typeof path !== "string" || !path.startsWith("/") || realpathSync(path) !== path || !lstatSync(path).isFile()) {
    throw new TypeError(`${label} is not a canonical physical file`);
  }
  const before = statSync(path, { bigint: true });
  const after = statSync(path, { bigint: true });
  if (before.dev !== after.dev || before.ino !== after.ino) throw new TypeError(`${label} identity changed during observation`);
  return Object.freeze({ path, device: String(after.dev), inode: String(after.ino) });
}

function exactHashArray(value: unknown, path: string, nonEmpty = false): readonly Hash[] {
  ...
  if (hashes.some((item, index) => index > 0 && hashes[index - 1]! >= item)) {
    throw new TypeError(`${path} must be strictly sorted and unique`);
  }
  return Object.freeze(hashes);
}

function eventContentSha256(event: RuntimeAcceptanceProcessEventV1): Hash {
  return sha256Hex(encodeCanonicalBytes(event));
}

function processEventsFromStore(
  store: SQLiteDurableStore,
  release: ReleaseIdentityV1,
): readonly RuntimeAcceptanceProcessEventV1[] {
  const namespace = `${NAMESPACE_PREFIX}:${release.releaseProvenanceHash.slice(2)}`;
  return Object.freeze(store.readAppendLog(namespace).map(record => {
    const event = decodeStoredEvent(decodeCanonicalJson(record.bytes), release);
    if (event.sequence !== record.sequence || event.eventId !== record.eventId
      || eventContentSha256(event) !== record.contentSha256) {
      throw new TypeError("pre-release restart process event durable row mismatch");
    }
    return event;
  }));
}

function exactRestartTriplet(events: readonly RuntimeAcceptanceProcessEventV1[]): Readonly<{
  readonly ready: RuntimeAcceptanceProcessEventV1;
  readonly observed: RuntimeAcceptanceProcessEventV1;
  readonly drained: RuntimeAcceptanceProcessEventV1;
}> {
  if (events.length < 3) throw new TypeError("pre-release restart lineage is incomplete");
  const [ready, observed, drained] = events;
  if (ready?.kind !== "aloha.runtime-process-ready"
    || observed?.kind !== "aloha.runtime-sigterm-observed"
    || drained?.kind !== "aloha.runtime-sigterm-drained"
    || observed.processReadyEventId !== ready.eventId
    || drained.sigtermObservedEventId !== observed.eventId
    || ready.processAnchorHash !== observed.processAnchorHash
    || ready.processAnchorHash !== drained.processAnchorHash
    || !sameCanonical(ready.release, observed.release)
    || !sameCanonical(ready.release, drained.release)) {
    throw new TypeError("pre-release ready to observed to drained lineage mismatch");
  }
  return Object.freeze({ ready, observed, drained });
}

function drainedRestartSnapshot(event: RuntimeAcceptanceProcessEventV1): Readonly<Record<string, unknown>> {
  const snapshot = event.checkpointRestartAfter;
  if (snapshot === null || typeof snapshot !== "object" || Array.isArray(snapshot)) {
    throw new TypeError("pre-release drained checkpoint snapshot is missing");
  }
  return snapshot as Readonly<Record<string, unknown>>;
}

function exactCheckpointStore(value: unknown, path: string): Readonly<{ readonly path: string; readonly device: string; readonly inode: string }> {
  if (value === null || typeof value !== "object" || Array.isArray(value)) throw new TypeError(`${path} must be an object`);
  const record = value as Readonly<Record<string, unknown>>;
  assertExactKeys(record, ["path", "device", "inode"], path);
  if (typeof record.path !== "string" || typeof record.device !== "string" || typeof record.inode !== "string") {
    throw new TypeError(`${path} fields must be strings`);
  }
  return Object.freeze({ path: record.path, device: record.device, inode: record.inode });
}

/** Seal the A terminal only from the three genuine, fsynced process events in
 * the same physical evidence database. The returned marker carries no runner,
 * acceptance, receipt, package, signing, or deployment authority. */
export function sealPreReleaseRestartTerminalV1(input: Readonly<{
  readonly databasePath: string;
  readonly authorization: PreReleaseLaunchAuthorizationV1;
  readonly authorizationClaim: PreReleaseAuthorizationClaimCapabilityV1;
}>): PreReleaseRestartTerminalV1 {
  assertExactKeys(input, ["databasePath", "authorization", "authorizationClaim"], "preReleaseRestartSeal");
  const authorization = decodePreReleaseLaunchAuthorizationV1(input.authorization);
  const claim = readFixedPreReleaseAuthorizationClaimV1(authorization, input.authorizationClaim);
  if (authorization.roundRole !== "restart-probe" || authorization.predecessor !== null
    || authorization.permissions.emitRestartMarker !== true) {
    throw new TypeError("only restart-probe authorization can seal the pre-release restart terminal");
  }
  const release = exactRelease({
    bindingId: authorization.runtimeBindingId,
    releaseProvenanceHash: authorization.releaseProvenanceHash,
    candidateReleaseCommit: authorization.candidateReleaseCommit,
  }, "preReleaseRestartSeal.release");
  const evidenceStore = physicalStore(input.databasePath, "pre-release process evidence database");
  const store = createSqliteDurableStore(evidenceStore.path);
  try {
    const evidenceAfterOpen = physicalStore(input.databasePath, "pre-release process evidence database after open");
    if (store.filename !== evidenceStore.path || !sameCanonical(evidenceAfterOpen, evidenceStore)) {
      throw new TypeError("pre-release process evidence database changed across SQLite open");
    }
    store.bindStoreRole(STORE_ROLE);
    const events = processEventsFromStore(store, release);
    if (events.length !== 3) throw new TypeError("restart-probe must contain exactly ready, observed, and drained process events");
    const { ready, observed, drained } = exactRestartTriplet(events);
    const snapshot = drainedRestartSnapshot(drained);
    const outcomeHashes = exactHashArray(drained.outcomeHashesAfter, "preReleaseRestart.outcomeHashesAfter", true);
    const flushedOutcomeHashes = exactHashArray(drained.flushedOutcomeHashes, "preReleaseRestart.flushedOutcomeHashes", true);
    if (!sameCanonical(outcomeHashes, flushedOutcomeHashes)) {
      throw new TypeError("pre-release restart terminal does not flush the complete outcome partition");
    }
    if (!Array.isArray(snapshot.outcomes) || !Array.isArray(snapshot.candidates) || !Array.isArray(snapshot.partials)
      || snapshot.partials.length !== 0) {
      throw new TypeError("pre-release restart terminal checkpoint partition is incomplete");
    }
    const recomputed = snapshot.outcomes.map((outcome, index) => candidateFinalOutcomeHash(
      outcome as Parameters<typeof candidateFinalOutcomeHash>[0],
    )).sort();
    if (!sameCanonical(recomputed, outcomeHashes)) {
      throw new TypeError("pre-release restart terminal outcome hashes do not match checkpoint rows");
    }
    const checkpointStore = exactCheckpointStore(snapshot.checkpointStore, "preReleaseRestart.checkpointStore");
    const payload = Object.freeze({
      schemaVersion: 1 as const,
      kind: "aloha.pre-release-restart-terminal" as const,
      sequence: "0" as const,
      release,
      ...
      authorizationClaimId: claim.claimId,
      processReadyEventId: ready.eventId,
      sigtermObservedEventId: observed.eventId,
      sigtermDrainedEventId: drained.eventId,
      processReadyContentSha256: eventContentSha256(ready),
      sigtermObservedContentSha256: eventContentSha256(observed),
      sigtermDrainedContentSha256: eventContentSha256(drained),
      processAnchorHash: ready.processAnchorHash,
      evidenceStore,
      checkpointStore,
      checkpointRevision: String(snapshot.checkpointRevision),
      checkpointRootEnvelopeHash: assertHash(snapshot.checkpointRootEnvelopeHash, "preReleaseRestart.checkpointRootEnvelopeHash"),
      runId: String(snapshot.runId),
      cutoff: canonicalClone(snapshot.cutoff),
      candidatePartitionRoot: assertHash(snapshot.candidatePartitionRoot, "preReleaseRestart.candidatePartitionRoot"),
      outcomePartitionRoot: assertHash(snapshot.outcomePartitionRoot, "preReleaseRestart.outcomePartitionRoot"),
      flushedOutcomeHashes,
    });
    if (!/^(0|[1-9][0-9]*)$/.test(payload.checkpointRevision) || payload.runId.length === 0) {
      throw new TypeError("pre-release restart terminal checkpoint identity is invalid");
    }
    const terminal = canonicalClone({ ...payload, terminalId: hashDomain(PRE_RELEASE_RESTART_DOMAIN, payload) }) as PreReleaseRestartTerminalV1;
    const bytes = encodeCanonicalBytes(terminal);
    const namespace = preReleaseRestartNamespace(release.releaseProvenanceHash, authorization.stagingManifestRoot);
    if (store.readAppendLog(namespace).length !== 0) throw new TypeError("pre-release restart terminal was already sealed");
    const receipt = store.appendFsyncMonotonic({
      namespace,
      sequence: "0",
      eventId: terminal.terminalId,
      contentSha256: sha256Hex(bytes),
      bytes,
    });
    if (receipt.fsynced !== true) throw new TypeError("pre-release restart terminal was not fsynced");
    return terminal;
  } finally {
    store.close();
  }
...
function decodeRestartTerminal(value: unknown): PreReleaseRestartTerminalV1 {
  if (value === null || typeof value !== "object" || Array.isArray(value)) throw new TypeError("pre-release restart terminal is invalid");
  const terminal = value as PreReleaseRestartTerminalV1;
  assertExactKeys(terminal, [
    "schemaVersion", "kind", "sequence", "release", "stagingArtifactSetRoot", "stagingManifestRoot", "controllerBoundaryEvidenceRoot",
    "authorizationId", "authorizationClaimId", "processReadyEventId", "sigtermObservedEventId",
    "sigtermDrainedEventId", "processReadyContentSha256", "sigtermObservedContentSha256",
    "sigtermDrainedContentSha256", "processAnchorHash", "evidenceStore", "checkpointStore",
    "checkpointRevision", "checkpointRootEnvelopeHash", "runId", "cutoff", "candidatePartitionRoot",
    "outcomePartitionRoot", "flushedOutcomeHashes", "terminalId",
  ], "preReleaseRestartTerminal");
  if (terminal.schemaVersion !== 1 || terminal.kind !== "aloha.pre-release-restart-terminal" || terminal.sequence !== "0") {
    throw new TypeError("pre-release restart terminal header is invalid");
  }
  const { terminalId: _terminalId, ...payload } = terminal;
  if (assertHash(terminal.terminalId, "preReleaseRestartTerminal.terminalId") !== hashDomain(PRE_RELEASE_RESTART_DOMAIN, payload)) {
    throw new TypeError("pre-release restart terminal id mismatch");
  }
  exactRelease(terminal.release, "preReleaseRestartTerminal.release");
  for (const field of [
    "stagingArtifactSetRoot", "stagingManifestRoot", "controllerBoundaryEvidenceRoot", "authorizationId", "authorizationClaimId",
    "processReadyEventId", "sigtermObservedEventId", "sigtermDrainedEventId", "processReadyContentSha256",
    "sigtermObservedContentSha256", "sigtermDrainedContentSha256", "processAnchorHash",
    "checkpointRootEnvelopeHash", "candidatePartitionRoot", "outcomePartitionRoot",
  ] as const) assertHash(terminal[field], `preReleaseRestartTerminal.${field}`);
  exactHashArray(terminal.flushedOutcomeHashes, "preReleaseRestartTerminal.flushedOutcomeHashes", true);
  exactCheckpointStore(terminal.evidenceStore, "preReleaseRestartTerminal.evidenceStore");
  exactCheckpointStore(terminal.checkpointStore, "preReleaseRestartTerminal.checkpointStore");
  return canonicalClone(terminal);
}

/** B-only physical predecessor gate. It reopens the same evidence database,
 * recomputes all A event bytes/IDs, joins the owner-signed predecessor, and
 * proves the exact non-empty flushed outcome partition remains visible in the
 * same physical checkpoint before any acceptance runner may be installed. */
export async function resolvePreReleaseRestartPredecessorV1(input: Readonly<{
  readonly databasePath: string;
  readonly authorization: PreReleaseLaunchAuthorizationV1;
  readonly authorizationClaim: PreReleaseAuthorizationClaimCapabilityV1;
  readonly checkpoint: CheckpointStore;
}>): Promise<PreReleaseRestartTerminalV1> {
  assertExactKeys(input, ["databasePath", "authorization", "authorizationClaim", "checkpoint"], "preReleaseRestartResolve");
  const authorization = decodePreReleaseLaunchAuthorizationV1(input.authorization);
  const authorizationClaim = readFixedPreReleaseAuthorizationClaimV1(
    authorization,
    input.authorizationClaim,
  );
  const checkpoint = assertIssuedCheckpointStore(input.checkpoint);
  if (authorization.roundRole !== "qualification-final" || authorization.predecessor === null) {
    throw new TypeError("only qualification-final authorization can resolve a pre-release predecessor");
  }
  if (!sameCanonical(authorizationClaim.predecessor, authorization.predecessor)) {
    throw new TypeError("qualification-final durable claim does not exact-bind the signed predecessor");
  }
  ...
  const release = exactRelease({
    ...
    releaseProvenanceHash: authorization.releaseProvenanceHash,
    candidateReleaseCommit: authorization.candidateReleaseCommit,
  }, "preReleaseRestartResolve.release");
  if (controllerReceipt.receiptId !== predecessor.controllerReceiptId
    || controllerReceipt.controller.implementationIdentityHash !== predecessor.controllerImplementationIdentityHash
    || controllerReceipt.pre.ready.processAnchorHash !== predecessor.targetProcessAnchorHash
    || hashProcessAnchor(controllerReceipt.pre.processAnchor) !== predecessor.targetProcessAnchorHash
    || controllerReceipt.post.ready.eventId !== predecessor.processReadyEventId
    || controllerReceipt.post.drained.eventId !== predecessor.sigtermDrainedEventId
    || controllerReceipt.post.terminal.terminalId !== predecessor.restartTerminalId
    || controllerReceipt.target.candidateReleaseCommit !== release.candidateReleaseCommit
    || controllerReceipt.target.runtimeBindingId !== release.bindingId
    || controllerReceipt.target.releaseProvenanceHash !== release.releaseProvenanceHash
    || controllerReceipt.target.stagingArtifactSetRoot !== authorization.stagingArtifactSetRoot
    || controllerReceipt.target.stagingManifestRoot !== authorization.stagingManifestRoot
    || controllerReceipt.target.controllerBoundaryEvidenceRoot !== authorization.controllerBoundaryEvidenceRoot
    || controllerReceipt.target.authorizationId !== predecessor.authorizationId
    || controllerReceipt.target.authorizationClaimId !== predecessor.authorizationClaimId) {
    throw new TypeError("qualification-final signed predecessor does not exact-join the fixed controller receipt");
  }
  const evidenceStore = physicalStore(input.databasePath, "pre-release process evidence database");
  const store = createSqliteDurableStore(evidenceStore.path);
  try {
    store.bindStoreRole(STORE_ROLE);
    const namespace = preReleaseRestartNamespace(release.releaseProvenanceHash, authorization.stagingManifestRoot);
    const markerRows = store.readAppendLog(namespace);
    if (markerRows.length !== 1) throw new TypeError("qualification-final requires exactly one durable restart terminal");
    const markerRow = markerRows[0]!;
    const terminal = decodeRestartTerminal(decodeCanonicalJson(markerRow.bytes));
    if (markerRow.sequence !== "0" || markerRow.eventId !== terminal.terminalId
      || markerRow.contentSha256 !== sha256Hex(markerRow.bytes)
      || !sameCanonical(terminal.release, release)
      || terminal.stagingArtifactSetRoot !== authorization.stagingArtifactSetRoot
      || terminal.stagingManifestRoot !== authorization.stagingManifestRoot
      || terminal.controllerBoundaryEvidenceRoot !== authorization.controllerBoundaryEvidenceRoot
      || terminal.authorizationId !== authorization.predecessor.authorizationId
      || terminal.authorizationClaimId !== authorization.predecessor.authorizationClaimId
      || terminal.processReadyEventId !== authorization.predecessor.processReadyEventId
      || terminal.sigtermDrainedEventId !== authorization.predecessor.sigtermDrainedEventId
      || terminal.terminalId !== authorization.predecessor.restartTerminalId
      || terminal.processAnchorHash !== authorization.predecessor.targetProcessAnchorHash
      || controllerReceipt.post.terminal.contentSha256 !== markerRow.contentSha256
      || !sameCanonical(terminal.evidenceStore, evidenceStore)) {
      throw new TypeError("qualification-final restart terminal predecessor binding mismatch");
    }
    const events = processEventsFromStore(store, release);
    if (events.length !== 4 || events[3]?.kind !== "aloha.runtime-process-ready") {
      throw new TypeError("qualification-final requires exactly one reopened process-ready event after A drain");
    }
    const { ready, observed, drained } = exactRestartTriplet(events.slice(0, 3));
    if (ready.eventId !== terminal.processReadyEventId
      || observed.eventId !== terminal.sigtermObservedEventId
      || drained.eventId !== terminal.sigtermDrainedEventId
      || eventContentSha256(ready) !== terminal.processReadyContentSha256
      || eventContentSha256(observed) !== terminal.sigtermObservedContentSha256
      || eventContentSha256(drained) !== terminal.sigtermDrainedContentSha256
      || events[3]!.processAnchorHash === terminal.processAnchorHash) {
      throw new TypeError("qualification-final physical A event lineage was mutated or not reopened by a new process");
    }
    const current = await checkpoint.loadRuntimeRestartSnapshot();
    const currentStore = exactCheckpointStore(current.checkpointStore, "preReleaseRestart.currentCheckpointStore");
    if (!sameCanonical(currentStore, terminal.checkpointStore)
      || current.checkpointRevision !== terminal.checkpointRevision
      || current.checkpointRootEnvelopeHash !== terminal.checkpointRootEnvelopeHash
      || current.runId !== terminal.runId
      || !sameCanonical(current.cutoff, terminal.cutoff)
      || current.candidatePartitionRoot !== terminal.candidatePartitionRoot) {
      throw new TypeError("qualification-final reopened checkpoint identity does not equal A drain");
    }
    if (current.partials.length !== 0 || current.outcomes.length === 0) {
      throw new TypeError("qualification-final reopened checkpoint outcome partition is incomplete");
    }
    const candidateKeys = new Set(current.candidates.map(candidate => candidate.familyCandidateKey));
    const outcomeKeys = new Set<string>();
    for (const outcome of current.outcomes) {
      if (!candidateKeys.has(outcome.familyCandidateKey) || outcomeKeys.has(outcome.familyCandidateKey)) {
        throw new TypeError("qualification-final reopened checkpoint contains an unbound outcome");
      }
      outcomeKeys.add(outcome.familyCandidateKey);
    }
    const currentHashes = current.outcomes.map(candidateFinalOutcomeHash).sort();
    if (!sameCanonical(currentHashes, terminal.flushedOutcomeHashes)
      || current.outcomePartitionRoot !== terminal.outcomePartitionRoot) {
      throw new TypeError("qualification-final did not recover A's complete flushed outcome partition");
    }
    return terminal;
  } finally {
    store.close();
